 ...
 *       dictionary, RoaringBitmap dependency sets, and row deduplication (default)</li>
 * </ul>
...
public class DependencyMap {
    ...
    /** Per-method dependencies: className#methodName → deps. Only populated in FULL_METHOD or FULL_MEMBER mode. */
    private final Map<String, Set<String>> methodDependencies;

    /** Per-test-class member-level deps: testClass → Set<"depClass#member">. Only populated in FULL_MEMBER mode. */
    private final Map<String, Set<String>> memberDependencies;

    /** Per-test-method member-level deps: testClass#method → Set<"depClass#member">. Only populated in FULL_MEMBER mode. */
    ...
    /** Store per-test-class member deps. Key: testClass, value: set of "depClass#memberName". */
    ...
    /** Store per-test-method member deps. Key: "testClass#method", value: set of "depClass#memberName". */
    ...
    /**
     * Returns all test classes whose dependency set intersects with the given changed classes.
     */
    public Set<String> getAffectedTests(Set<String> changedClasses) {
        Set<String> affected = new LinkedHashSet<>();
        if (changedClasses.isEmpty()) return affected;
    ...
    /**
     * Saves in V3 binary format (LZ4-compressed, trie + RoaringBitmaps, row-deduped,
    ...
    private static DependencyMap loadBinary(Path indexFile) throws IOException {
        try (InputStream fis = Files.newInputStream(indexFile);
            ...
            ClassNameTrie trie = ClassNameTrie.readFrom(
                    new DataInputStream(new ByteArrayInputStream(trieBytes)));
            ...
            DependencyMap map = new DependencyMap();
            @SuppressWarnings("unchecked") Set<String>[] depSets = new Set[testCount];
        ...
        try (Stream<Path> files = Files.list(depsDir)) {
            for (Path file : files.toList()) {
                ...
                if (fileName.endsWith(".deps")) {
                    ...
                    Set<String> deps = Files.readAllLines(file).stream()
                            .map(String::trim)
                            .filter(s -> !s.isEmpty())
                            .collect(Collectors.toCollection(TreeSet::new));
                    map.put(testClass, deps);
                    ...
                    if (!lines.isEmpty() && lines.get(0).startsWith("# ")) {
                        ...
                        Set<String> deps = lines.stream().skip(1)
                                .map(String::trim)
     ...
     * Called from UsageStore (on the bootstrap classpath) via reflection at JVM shutdown.
     * Loads the existing index (if present), merges new entries, and saves back.
     * 
     * <p><b>Performance note:</b> This method is synchronized but called at most once per test fork.
     * When the agent is configured with outputDir, UsageStore writes incremental .deps files instead,
     * allowing multiple forks to run in parallel without contention.
     */
    public static synchronized void mergeFromAgent(Path indexFile, Map<String, Set<String>> deps,
                                                    Map<String, Set<String>> methodDeps,
                                                    Map<String, Set<String>> memberDeps,
        ...
        // warn if no actual dependencies were captured (common with groupId/package mismatch)
    ...
    /**
     * High-performance parallel aggregation from multiple .deps files.
     * Scans a directory for *.deps and *.mdeps files, loads them in parallel,
     * and merges into a single DependencyMap, finally saving to indexFile.
     * 
     * <p>This method is optimized for the post-fork aggregation phase:
     * <ul>
    ...
    public static void aggregateFromDepsDirectory(Path depsDir, Path indexFile) throws IOException {
        ...
        var mdepTasks = mdepsFiles.parallelStream().map(mdepFile ->
            pool.submit(() -> {
                try {
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof DependencyMap other)) return false;
