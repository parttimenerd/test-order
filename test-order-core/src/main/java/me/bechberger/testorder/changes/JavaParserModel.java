...
/**
 * Alternative parser backend using JavaParser instead of the island-grammar regex parser.
 * <p>
 * Activated via {@code testorder.parser.mode=javaparser}.
 * Produces the same {@link SourceFileModel.Model} output, so all downstream code
 * (StructuralDiff, scoring, etc.) works unchanged.
 */
class JavaParserModel {
    ...
    private static void visitType(
            TypeDeclaration<?> td, String packageName, String outerFqcn,
            String stripped, String source,
            SourceFileModel.Detail detail,
            List<SourceFileModel.TypeNode> types,
            ...
            if (searchFrom >= 0) {
                ...
                if (bodyStart >= 0 && bodyStart < stripped.length()) {
                    ...
                    if (bodyEnd >= 0) {
                        ...
                        compactBody = SourceFileModel.removeCommentsAndEmptyLines(
        ...
        types.add(new SourceFileModel.TypeNode(kind, simpleName, fqcn,
                bodyStart, bodyEnd, signature, bodyText, compactBody));
        ...
        for (var member : td.getMembers()) {
            if (member instanceof TypeDeclaration<?> nested) {
                visitType(nested, packageName, fqcn, stripped, source, detail,
                        types, methods, fields, initializers);
            }
        }
    }

    private static void visitMethod(MethodDeclaration md, String fqcn,
                                    String stripped, String source,
                                    List<SourceFileModel.MethodNode> methods) {
        ...
        methods.add(new SourceFileModel.MethodNode(name, fqcn, false, isAbstract,
                bodyText, bodyHash, compactBody));
    }

    private static void visitConstructor(ConstructorDeclaration cd, String fqcn,
                                         String stripped, String source,
                                         List<SourceFileModel.MethodNode> methods) {
        ...
        methods.add(new SourceFileModel.MethodNode(name, fqcn, true, false,
                bodyText, bodyHash, compactBody));
    }

    private static void visitField(FieldDeclaration fd, String fqcn,
                                   String stripped,
                                   List<SourceFileModel.FieldNode> fields) {
    ...
    private static void visitInitializer(InitializerDeclaration id, String fqcn,
                                         String stripped,
                                         List<SourceFileModel.InitializerNode> initializers) {
    ...
    private static SourceFileModel.TypeKind typeKindOf(TypeDeclaration<?> td) {
        if (td instanceof EnumDeclaration) return SourceFileModel.TypeKind.ENUM;
        if (td instanceof RecordDeclaration) return SourceFileModel.TypeKind.RECORD;
        if (td instanceof AnnotationDeclaration) return SourceFileModel.TypeKind.ANNOTATION;
        if (td instanceof ClassOrInterfaceDeclaration coid) {
