 ...
 * <p>
 * Larger compressed sizes indicate more complex / information-dense source files,
 * which are more likely to harbour subtle bugs after modification.  The scores
 * are normalised to [0.0, 1.0] relative to the largest changed file.
 * <p>
...
public class ChangeComplexity {
     ...
     *
     * @param changedClasses FQCNs of changed application classes
     * @param sourceRoots    source root directories to search for source files
     * @return FQCN → normalised complexity (0.0–1.0); classes whose source
    ...
    /**
     * Computes normalized complexity scores, optionally refined by structural change data.
     * <p>
     * When {@code changedMembers} is provided, the score blends compressed file size
     * (information density) with the number of changed members in each class (change breadth).
     * This avoids over-scoring a 1-line change in a huge file, and under-scoring a
     * multi-method rewrite in a small file.
     *
     * @param changedClasses  FQCNs of changed application classes
     * @param sourceRoots     source root directories to search for source files
     * @param changedMembers  structural analysis result (may be null for backward compat)
    ...
    public static Map<String, Double> compute(Set<String> changedClasses, List<Path> sourceRoots,
        ...
        for (String fqcn : changedClasses) {
            ...
            try {
                ...
                if (compressed > maxSize) maxSize = compressed;
        ...
        if (changedMembers != null && !changedMembers.membersByClass().isEmpty()) {
            ...
            if (maxMembers > 0) {
                for (String fqcn : result.keySet()) {
                    Set<String> members = changedMembers.membersByClass().get(fqcn);
                    double memberRatio = (members != null)
                            ? (double) members.size() / maxMembers
                            : 0.0;
     ...
     * <p>
     * For each changed class, collects the LCS-based diff text (insertions + deletions)
     * across all its modified/added/removed members, compresses it with Deflate, and
     * uses the compressed size as the raw complexity value.  More complex, information-dense
     * diffs compress less well and thus score higher.
     * <p>
     * Normalised to [0.0, 1.0] relative to the most-complex changed class.
     *
     * @param diffs the structural diffs (must include body changes)
    ...
    public static Map<String, Double> computeFromDiffs(List<StructuralDiff.FileDiff> diffs) {
        if (diffs == null || diffs.isEmpty()) return Map.of();
        ...
        for (StructuralDiff.FileDiff diff : diffs) {
            for (StructuralDiff.BodyChange bc : diff.bodyChanges()) {
                ...
                if (!diffText.isEmpty()) {
                    diffTextPerClass.computeIfAbsent(bc.fqcn(), k -> new StringBuilder())
                            .append(diffText).append("\n");
                }
            }
        }

        if (diffTextPerClass.isEmpty()) return Map.of();
     ...
     * @param changedClasses  FQCNs of changed application classes
     * @param sourceRoots     source root directories to search for source files
     * @param changedMembers  structural analysis result (may be null)
     * @param diffs           structural diffs with body changes (may be null)
     * @return FQCN → normalised complexity (0.0–1.0)
     */
    public static Map<String, Double> compute(Set<String> changedClasses, List<Path> sourceRoots,
                                               StructuralChangeAnalyzer.ChangedMembers changedMembers,
                                               List<StructuralDiff.FileDiff> diffs) {
     ...
     * Useful when the caller already has the raw values (e.g. from config properties).
     */
    public static Map<String, Double> fromRawSizes(Map<String, Integer> rawSizes) {
        if (rawSizes.isEmpty()) return Map.of();
        int maxSize = 0;
    ...
    /**
     * Resolves a FQCN to a source file under the given roots.
     * Searches for both .java and .kt files. Returns {@code null} if not found.
     */
    ...
    /**
     * Serialises a complexity map to a property-style string: {@code fqcn:score,fqcn:score,...}
     * Stores normalised values (0.0–1.0) as produced by {@link #compute}.
     */
    public static String serialise(Map<String, Double> complexity) {
        if (complexity.isEmpty()) return "";
    ...
    public static Map<String, Double> deserialise(String value) {
        ...
        for (String pair : value.split(",")) {
            ...
            if (colon > 0 && colon < pair.length() - 1) {
                try {
                    result.put(pair.substring(0, colon),
