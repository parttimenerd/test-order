...
public class MethodHashStore {
    ...
    public static MethodHashStore scan(Path testSourceRoot) throws IOException {
        ...
        try (Stream<Path> walk = Files.walk(testSourceRoot)) {
            for (Path file : walk.filter(p -> Files.isRegularFile(p)
                    && SourceFileModel.isSourceFile(p.toString())).toList()) {
                try {
                    String source = Files.readString(file);
                    String pkg = SourceFileModel.extractPackageName(source);
                    hashes.putAll(SourceFileModel.parse(source, pkg, SourceFileModel.Detail.METHODS)
                            .methodHashes());
                } catch (Exception e) {
    ...
    /**
     * Returns method keys ({@code className#methodName}) that have changed,
     * been added, or been deleted compared to a previous snapshot.
     */
    ...
    public boolean equals(Object o) {
        ...
        if (!(o instanceof MethodHashStore other)) return false;
