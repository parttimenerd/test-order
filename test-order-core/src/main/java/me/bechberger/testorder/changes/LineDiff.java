 ...
 * Computes the number of changed lines and the actual diff text between two
 * texts.
 */
public class LineDiff {
	...
	/**
	 * Computes the number of changed lines between two text blocks. Uses LCS
	 * (longest common subsequence) to determine the edit distance in lines.
	 *
	...
	public static int changedLineCount(String oldText, String newText) {
		...
		String[] oldLines = oldText.isEmpty() ? new String[0] : oldText.split("\\R", -1);
		String[] newLines = newText.isEmpty() ? new String[0] : newText.split("\\R", -1);
	 ...
	 * Uses LCS with O(m×n) traceback to identify changed lines precisely. Each line
	 * is prefixed with "+" (insertion) or "-" (deletion).
	 *
	...
	/**
	 * Computes LCS length using O(min(m,n)) space DP. For bodies under a few
	 * hundred lines this is efficient enough.
	 */
