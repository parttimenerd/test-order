 ...
 * <p>
 * Exploits package-prefix redundancy for compact serialization.
 * IDs are assigned via DFS traversal so classes in the same package
 * get contiguous IDs (good for RoaringBitmap run containers).
 */
class ClassNameTrie {
    ...
    private void assignIdsDfs(Node node, StringBuilder prefix, List<String> names,
    ...
    int getId(String name) {
        ...
        if (id == null) throw new NoSuchElementException("Unknown class name: " + name);
        return id;
    }
    ...
    private static void rebuildLookup(Node node, StringBuilder prefix,
                                      List<String> names, Map<String, Integer> map) {
        int prevLen = prefix.length();
        prefix.append(node.label);
        if (node.terminal) {
            String name = prefix.toString();
            // ensure list is large enough (IDs may not arrive in order from trie)
            while (names.size() <= node.classId) names.add(null);
