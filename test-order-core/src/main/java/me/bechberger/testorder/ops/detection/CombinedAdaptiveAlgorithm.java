...
/**
 * Algorithm 8: Combined Adaptive detection.
 * The primary detection algorithm that intelligently combines multiple strategies
 * with an "isolate first" protocol and adaptive budget allocation.
 *
 * <p>Strategy:
 * <ol>
 ...
 *   <li>For each discovered victim, immediately isolate the polluter via ddmin</li>
 *   <li>Adapt strategy based on findings (more brittles → switch to PFAST-style)</li>
 * </ol>
 */
public class CombinedAdaptiveAlgorithm implements DetectionAlgorithm {
    ...
    public List<ODResult> detect(DetectionContext ctx) {
        ...
        // (works even without execution order control, e.g. when Surefire picks its own order)
        addInitialExclusionProbes(ctx, workQueue, knowledge);

        // Phase 3: Adaptive work loop
        int runsUsed = 0;
        int maxRuns = estimatedRuns(ctx.referenceOrder().size(),
                ctx.graph() != null ? ctx.graph().edgeCount() : 0);
        ...
        while (!workQueue.isEmpty() && !ctx.timeBudgetExhausted() && runsUsed < maxRuns) {
            ...
            long brittleCount = findings.stream()
                    .filter(r -> r.type() == ODType.BRITTLE).count();
            if (brittleCount > 2 && workQueue.stream()
                    .noneMatch(a -> a.type == ActionType.EXCLUSION_PROBE)) {
        ...
        while (!ctx.timeBudgetExhausted() && runsUsed < maxRuns) {
            ...
            for (String failed : result.failedTests()) {
                ...
                if (!tk.confirmedPolluters().isEmpty()) continue; // Already resolved
        ...
        if (ctx.state() != null && ctx.state().runs().size() >= 3) {
            ...
            for (ODResult hr : histResults) {
                ...
                if (hr.dependencyChain().size() >= 2) {
                    ...
                    workQueue.add(new Action(ActionType.ISOLATE_PAIR, 7.5,
                            hr.victim(), List.of(polluter)));
                }
            }
        }
    }
    ...
    private void seedFromConflictGraph(DetectionContext ctx, PriorityQueue<Action> workQueue,
        ...
        for (ConflictEdge edge : edges) {
            if (workQueue.size() > 200) break; // Cap initial queue size

            // For each edge, schedule probes for both directions
            double priority = 5.0 + edge.weight() * 3.0; // Higher weight → higher priority
            workQueue.add(new Action(ActionType.PROBE_EDGE, priority,
                    edge.testB(), List.of(edge.testA())));
            workQueue.add(new Action(ActionType.PROBE_EDGE, priority - 0.1,
                    edge.testA(), List.of(edge.testB())));
        }
    }

    private int executeAction(Action action, DetectionContext ctx,
                              PriorityQueue<Action> workQueue,
                              Map<String, TestKnowledge> knowledge,
                              List<ODResult> findings, Random rng) {
    ...
    private int executeIsolatePair(Action action, DetectionContext ctx,
                                   TestKnowledge tk, List<ODResult> findings) {
        if (action.candidates.isEmpty()) return 0;
        ...
        if (result.failed(action.victim)) {
            ...
            findings.add(new ODResult(action.victim, ODType.VICTIM,
                    List.of(suspect, action.victim),
                    "Confirmed: " + suspect + " pollutes " + action.victim,
                    0.99));
        } else {
    ...
    private int executeConfirmBrittle(Action action, DetectionContext ctx,
                                      TestKnowledge tk, List<ODResult> findings) {
        if (action.candidates.isEmpty()) return 0;
        ...
        if (result.passed(action.victim)) {
            ...
            findings.add(new ODResult(action.victim, ODType.BRITTLE,
                    List.of(setter, action.victim),
                    "Confirmed brittle: " + action.victim + " needs " + setter,
                    0.99));
        }
        return 1;
    }

    private int executeMinimize(Action action, DetectionContext ctx,
                                TestKnowledge tk, PriorityQueue<Action> workQueue,
        ...
        if (!minimal.isEmpty()) {
            ...
            if (isolation.passed(action.victim)) {
                ...
                findings.add(new ODResult(action.victim, ODType.VICTIM,
                        buildChain(minimal, action.victim),
                        "Minimized: " + minimal + " → " + action.victim,
                        0.95));
            } else {
                ...
                if (refIdx > 0) {
                    ...
                    workQueue.add(new Action(ActionType.CONFIRM_BRITTLE, 8.5,
                            action.victim, List.of(possibleSetter)));
                }
            }
        }
    ...
    private int executeProbeEdge(Action action, DetectionContext ctx,
                                 TestKnowledge tk, PriorityQueue<Action> workQueue,
                                 Map<String, TestKnowledge> knowledge) {
        if (action.candidates.isEmpty()) return 0;
        String suspect = action.candidates.get(0);
        ...
        if (result.failed(action.victim)) {
            ...
            if (isolation.passed(action.victim)) {
                ...
                workQueue.add(new Action(ActionType.ISOLATE_PAIR, 9.5,
                        action.victim, List.of(suspect)));
                return 2;
            } else {
                tk.setPassesAlone(false);
                return 2;
    ...
    private int executeExclusionProbe(Action action, DetectionContext ctx,
                                      PriorityQueue<Action> workQueue,
                                      Map<String, TestKnowledge> knowledge) {
        if (action.candidates.isEmpty()) return 0;
        ...
        for (String failed : result.failedTests()) {
            ...
            workQueue.add(new Action(ActionType.CONFIRM_BRITTLE, 9.0,
                    failed, List.of(excluded)));
        }
        return 1;
    }
    ...
    private void addExclusionProbes(DetectionContext ctx, PriorityQueue<Action> workQueue,
        ...
        for (int i = 0; i < limit; i++) {
            ...
            workQueue.add(new Action(ActionType.EXCLUSION_PROBE, 4.0,
                    test, List.of(test)));
        }
    }
    ...
    private void addInitialExclusionProbes(DetectionContext ctx, PriorityQueue<Action> workQueue,
        ...
        for (int i = 0; i < limit; i++) {
            String test = ctx.referenceOrder().get(i);
            workQueue.add(new Action(ActionType.EXCLUSION_PROBE, 15.0,
                    test, List.of(test)));
        }
    }
