...
public class MethodLevelDetection {
	 ...
	 */
	public record MethodODResult(String testClass, String victimMethod, ODType type, List<String> dependencyChain,
	 ...
	 *
	 * @param testClass
	 *            FQCN of the test class
	 * @param methods
	 *            list of test method names in this class
	 * @param runner
	 *            test runner that supports method ordering
	 * @param depMap
	 *            dependency map (nullable, used for prioritization)
	 * @param deadline
	 ...
	 */
	public static List<MethodODResult> detect(String testClass, List<String> methods, TestRunner runner,
			DependencyMap depMap, long deadline, Random rng) {
		if (methods.size() < 2) {
			return List.of();
		}
		...
		for (String victim : victimCandidates) {
			...
			if (polluter != null) {
				findings.add(new MethodODResult(testClass, victim, ODType.VICTIM, List.of(polluter, victim),
						"Method '" + victim + "' fails when '" + polluter + "' runs before it", 0.95));
			} else if (!predecessors.isEmpty()) {
				// Couldn't isolate but confirmed failure — report with lower confidence
				findings.add(
						new MethodODResult(testClass, victim, ODType.VICTIM,
		...
		if (System.currentTimeMillis() < deadline) {
			for (String method : passingMethods) {
				if (System.currentTimeMillis() >= deadline)
					break;
				if (victimCandidates.contains(method))
				...
				if (isolation.failed(method)) {
					...
					if (setter != null) {
						findings.add(new MethodODResult(testClass, method, ODType.BRITTLE, List.of(setter, method),
								"Method '" + method + "' requires '" + setter + "' to run first", 0.95));
					} else {
						findings.add(new MethodODResult(testClass, method, ODType.BRITTLE, List.of(method),
								"Method '" + method + "' fails in isolation (needs a state-setter)", 0.8));
					}
				}
			}
		}
		...
		if (depMap != null && depMap.hasMethodDeps() && System.currentTimeMillis() < deadline) {
			Set<String> alreadyFound = findings.stream().map(MethodODResult::victimMethod).collect(Collectors.toSet());
			List<MethodODResult> depGuided = dependencyGuidedProbes(testClass, methods, passingMethods, alreadyFound,
					depMap, runner, deadline, rng);
	 ...
	 */
	private static String isolatePolluter(String testClass, String victim, List<String> predecessors,
			Set<String> passingMethods, TestRunner runner, long deadline) {
		if (predecessors.isEmpty())
			return null;
		if (predecessors.size() == 1) {
			// Verify: run [predecessor, victim] — if victim fails, predecessor is the
			// polluter
			TestRunner.MethodRunResult verify = runner.runMethods(testClass, List.of(predecessors.get(0), victim));
			return verify.failed(victim) ? predecessors.get(0) : null;
		}

		// Binary search: split predecessors, run [left half + victim] vs [right half +
		// victim]
	 ...
	 */
	private static String isolateSetter(String testClass, String brittle, List<String> referenceOrder,
		...
		if (predecessors.size() == 1) {
			TestRunner.MethodRunResult verify = runner.runMethods(testClass, List.of(predecessors.get(0), brittle));
	...
	private static String isolateSetterInRange(String testClass, String brittle, List<String> candidates,
			TestRunner runner, long deadline) {
		if (candidates.isEmpty())
			return null;
		if (candidates.size() == 1) {
			TestRunner.MethodRunResult verify = runner.runMethods(testClass, List.of(candidates.get(0), brittle));
	 ...
	 */
	private static List<MethodODResult> dependencyGuidedProbes(String testClass, List<String> methods,
			Set<String> passingMethods, Set<String> alreadyFound, DependencyMap depMap, TestRunner runner,
			long deadline, Random rng) {
		List<MethodODResult> findings = new ArrayList<>();

		// Find pairs of methods that share member-level dependencies (potential state
		// conflicts)
		...
		for (int i = 0; i < methodsWithDeps.size() && System.currentTimeMillis() < deadline; i++) {
			for (int j = i + 1; j < methodsWithDeps.size() && System.currentTimeMillis() < deadline; j++) {
				...
				if (alreadyFound.contains(a) && alreadyFound.contains(b))
					continue;
				...
				if (!alreadyFound.contains(b) && passingMethods.contains(b)) {
					...
					if (abResult.failed(b)) {
						findings.add(new MethodODResult(testClass, b, ODType.VICTIM, List.of(a, b),
								"Method '" + b + "' fails when '" + a + "' runs before it " + "(shared members: "
										+ shared.stream().limit(3).collect(Collectors.joining(", ")) + ")",
								0.95));
						alreadyFound.add(b);
					}
				}
				...
				if (!alreadyFound.contains(a) && passingMethods.contains(a)) {
					...
					if (baResult.failed(a)) {
						findings.add(new MethodODResult(testClass, a, ODType.VICTIM, List.of(b, a),
								"Method '" + a + "' fails when '" + b + "' runs before it " + "(shared members: "
										+ shared.stream().limit(3).collect(Collectors.joining(", ")) + ")",
								0.95));
						alreadyFound.add(a);
					}
