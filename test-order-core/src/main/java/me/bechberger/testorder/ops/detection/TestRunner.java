...
/**
 * Abstraction for running tests in a specific order and comparing results to
 * baseline. Implementations delegate to Surefire/Failsafe/Gradle test task.
 */
public interface TestRunner {
	 ...
	 *
	 * @param testOrder
	 *            ordered list of test class FQCNs to execute
	 * @return the run result with pass/fail per test
	 */
	TestRunResult run(List<String> testOrder);

	/**
	 * Run a single test class with methods in the specified order. Returns
	 * method-level pass/fail results.
	 *
	 * @param testClass
	 *            FQCN of the test class
	 * @param methodOrder
	 *            ordered list of method names to execute
	 ...
	 * level. After this completes, the dependency index file should be updated with
	 * fresh data.
	 *
	...
	/**
	 * Set a deadline (epoch millis) after which the runner should kill any
	 * in-progress subprocess and return a failure result. Use
	 * {@link Long#MAX_VALUE} for no deadline.
	 *
	 ...
	 */
	record MethodRunResult(String testClass, List<String> executionOrder, Set<String> passedMethods,
