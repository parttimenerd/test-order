 ...
 * Uses the conflict graph to batch-test edge clusters, then narrows with ddmin.
 */
public class IterativeRefinementAlgorithm implements DetectionAlgorithm {
    ...
    public List<ODResult> detect(DetectionContext ctx) {
        ...
        if (ctx.graph() == null || ctx.graph().isEmpty()) return findings;
        ...
        for (List<ConflictEdge> cluster : clusters) {
            ...
            if (result.allPassed()) continue; // Cluster is benign
            ...
            for (String failed : result.failedTests()) {
                ...
                List<String> minimalPolluters = DeltaDebugging.minimize(
                        predecessors, failed, ctx.runner(), 10);
                ...
                if (!minimalPolluters.isEmpty()) {
                    ...
                    if (isolation.passed(failed)) {
                        findings.add(new ODResult(failed, ODType.VICTIM,
                                buildChain(minimalPolluters, failed),
                                "Iterative refinement: " + minimalPolluters + " → " + failed,
                                0.95));
                    }
                }
            }
    ...
    private List<String> generateViolation(List<ConflictEdge> cluster, List<String> reference) {
        // Strategy: for each edge (A, B), ensure B comes before A (reverse their relative order)
        ...
        // Place involved tests in reverse reference order, then others in reference order
