...
/**
 * Algorithm 7: PFAST Single-Exclusion. Detects BRITTLE tests by systematically
 * removing one test at a time. If removing test X causes test Y to fail, Y is
 * BRITTLE and needs X as a setter.
 */
			...
			for (String failed : result.failedTests()) {
				...
				if (verify.passed(failed)) {
					...
					findings.add(new ODResult(failed, ODType.BRITTLE, List.of(excluded, failed),
							"Brittle: " + failed + " needs " + excluded + " as state-setter", 0.95));
				}
			}
		}
