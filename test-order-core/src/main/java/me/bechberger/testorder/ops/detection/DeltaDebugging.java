...
/**
 * Delta debugging (ddmin) implementation.
 * Finds the 1-minimal subset of tests that causes a victim to fail.
 */
public final class DeltaDebugging {
    ...
    /**
     * Find the minimal set of tests from {@code candidates} that, when placed before
     * {@code victim}, cause it to fail.
     *
     * @param candidates potential polluter tests
     * @param victim the test that fails
     * @param runner test execution engine
     * @param runBudget maximum number of runs to spend
     * @return minimal polluter set, or empty if none found within budget
     */
    public static List<String> minimize(List<String> candidates, String victim,
                                        TestRunner runner, int runBudget) {
    ...
    private static List<String> ddmin(List<String> candidates, String victim,
                                      TestRunner runner, int partitions,
                                      int runBudget) {
        ...
        for (int i = 0; i < chunks.size(); i++) {
            ...
            if (result.failed(victim)) {
                ...
                return ddmin(complement, victim, runner,
                        Math.max(2, partitions - 1), runBudget);
            }
        }

        // Neither chunks nor complements work alone — increase granularity
        if (partitions < candidates.size()) {
            return ddmin(candidates, victim, runner,
                    Math.min(candidates.size(), partitions * 2), runBudget);
        }
