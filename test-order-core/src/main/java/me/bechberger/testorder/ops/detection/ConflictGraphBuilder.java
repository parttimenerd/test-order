...
/**
 * Builds a conflict graph from DependencyMap data.
 * Two tests are connected by an edge if they share access to the same mutable fields.
 */
public final class ConflictGraphBuilder {
     ...
     *
     * @param depMap dependency map with member-level or class-level data
     * @param state run history for intermittency boosting
     * @param testClasses tests to consider
     * @return conflict graph with weighted edges
     */
    public static ConflictGraph build(DependencyMap depMap, TestOrderState state,
    ...
    private static List<ConflictEdge> buildFromMemberDeps(DependencyMap depMap,
                                                          List<String> testClasses,
    ...
    private static List<ConflictEdge> buildFromClassDeps(DependencyMap depMap,
                                                         List<String> testClasses,
                                                         Set<String> intermittent) {
        ...
        for (Map.Entry<String, Set<String>> entry : classToTests.entrySet()) {
            Set<String> tests = entry.getValue();
            if (tests.size() < 2 || tests.size() > testClasses.size() / 2) continue;
        ...
        for (Map.Entry<String, Integer> entry : sharedCount.entrySet()) {
            ...
            edgeMap.put(entry.getKey(),
                    new ConflictEdge(parts[0], parts[1], Set.of(), weight));
        }

        return new ArrayList<>(edgeMap.values());
    }

    private static double computeWeight(int sharedCount, Set<String> intermittent,
                                        String testA, String testB) {
    ...
    private static Set<String> findIntermittentTests(TestOrderState state) {
        if (state == null || state.runs().size() < 3) return Set.of();
        ...
        for (TestOrderState.RunRecord run : state.runs()) {
            for (TestOrderState.TestOutcome outcome : run.outcomes()) {
                ...
                if (outcome.failed()) flags[0] = true;
                else flags[1] = true;
            }
        }
