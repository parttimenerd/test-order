...
/**
 * Searches for a "cleaner" test that neutralizes a polluter's effect.
 * Given (polluter → victim), finds a test C such that [polluter, C, victim] passes.
 */
public final class CleanerSearch {
     ...
     *
     * @param polluter the test that pollutes state
     * @param victim the test that fails after polluter
     * @param candidates potential cleaner tests
     ...
     */
    public static Optional<String> find(String polluter, String victim,
        ...
        for (String candidate : candidates) {
            ...
            if (candidate.equals(polluter) || candidate.equals(victim)) continue;
     ...
     */
    public static List<String> findAll(String polluter, String victim,
                                       List<String> candidates, TestRunner runner,
                                       int runBudget) {
        List<String> cleaners = new ArrayList<>();

        TestRunResult baseline = runner.run(List.of(polluter, victim));
        if (!baseline.failed(victim)) return cleaners;
        runBudget--;
