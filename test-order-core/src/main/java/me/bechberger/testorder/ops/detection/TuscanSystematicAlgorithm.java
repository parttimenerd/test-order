...
/**
 * Algorithm 5: Tuscan Square Systematic Coverage.
 * Uses combinatorial scheduling (Tuscan squares) to guarantee every
 * pair of tests (A, B) appears with A immediately before B in some run.
 * This maximizes coverage of pairwise interactions.
 */
public class TuscanSystematicAlgorithm implements DetectionAlgorithm {
    ...
    public List<ODResult> detect(DetectionContext ctx) {
        ...
        // Generate runs using circular-shift construction (approximation of Tuscan square)
        ...
        for (int row = 0; row < n && !ctx.timeBudgetExhausted(); row++) {
            ...
            for (String failed : result.failedTests()) {
                ...
                if (knownVictims.contains(failed)) continue;
                ...
                if (suspect != null) {
                    ...
                    if (verify.failed(failed)) {
                        findings.add(new ODResult(failed, ODType.VICTIM,
                                List.of(suspect, failed),
                                "Tuscan: " + suspect + " immediately before " + failed
                                        + " causes failure (row " + row + ")",
                                0.9));
                        continue;
                    }
                }
                ...
                List<String> chain = new ArrayList<>(predecessors.subList(
                        Math.max(0, predecessors.size() - 3), predecessors.size()));
                chain.add(failed);
                findings.add(new ODResult(failed, ODType.VICTIM, chain,
                        "Tuscan row " + row + ": failed with unknown polluter subset",
                        0.6));
            }
        }
