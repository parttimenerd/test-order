...
/**
 * Algorithm 3: Reverse Order detection.
 * Runs the test suite in reverse order — cheapest possible detection (1 run).
 * DTDetector reports ~66% of OD bugs are detected by a single reverse run.
 */
public class ReverseOrderAlgorithm implements DetectionAlgorithm {
    ...
    public List<ODResult> detect(DetectionContext ctx) {
        ...
        for (String failed : result.failedTests()) {
            ...
            if (!predecessors.isEmpty() && !ctx.timeBudgetExhausted()) {
                ...
                if (!minimal.isEmpty()) {
                    ...
                    findings.add(new ODResult(failed, ODType.VICTIM,
                            buildChain(minimal, failed),
                            "Reverse order: " + polluter + " pollutes " + failed,
                            0.95));
                    continue;
                }
            }
            ...
            findings.add(new ODResult(failed, ODType.VICTIM,
                    List.of(failed),
                    "Failed in reverse order — OD candidate (polluter could not be isolated)",
                    0.6));
        }
