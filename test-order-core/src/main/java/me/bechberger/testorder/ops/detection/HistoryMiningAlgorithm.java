...
/**
 * Algorithm 6: History Mining.
 * Zero-cost algorithm that mines existing run history to find OD suspects
 * using RankFO-style differential scoring.
 */
public class HistoryMiningAlgorithm implements DetectionAlgorithm {
    ...
    public List<ODResult> detect(DetectionContext ctx) {
        ...
        for (TestOrderState.RunRecord run : ctx.state().runs()) {
            List<String> order = run.outcomes().stream()
                    .map(TestOrderState.TestOutcome::testClass)
                    .toList();

            for (TestOrderState.TestOutcome outcome : run.outcomes()) {
                outcomes.computeIfAbsent(outcome.testClass(), k -> new ArrayList<>())
                        .add(outcome.failed());
        ...
        // RankFO: for each intermittent test, rank which predecessors correlate with failure
        for (Map.Entry<String, List<Boolean>> entry : outcomes.entrySet()) {
            ...
            if (failCount == 0 || passCount == 0) continue; // Not intermittent

            List<List<String>> failPredecessors = predecessorMap.getOrDefault(test, List.of());
            if (failPredecessors.isEmpty()) continue;

            // Score each predecessor by (appears-in-fail-runs / fail-count) - (appears-in-pass-runs / pass-count)
            ...
            scores.entrySet().stream()
                    ...
                    .ifPresent(best -> findings.add(new ODResult(
                            test, ODType.VICTIM,
                            List.of(best.getKey(), test),
    ...
    private Map<String, Double> rankFO(String victim, List<TestOrderState.RunRecord> runs) {
        ...
        for (TestOrderState.RunRecord run : runs) {
            List<String> order = run.outcomes().stream()
                    .map(TestOrderState.TestOutcome::testClass).toList();
            int victimIdx = order.indexOf(victim);
            if (victimIdx < 0) continue;

            boolean failed = run.outcomes().stream()
                    .anyMatch(o -> o.testClass().equals(victim) && o.failed());
        ...
        for (String p : allPredecessors) {
            ...
            double passRate = passRuns > 0
                    ? appearsInPass.getOrDefault(p, 0) / (double) passRuns : 0.0;
