...
/**
 * Algorithm 2: Random Reordering detection.
 * Runs the suite in random orders to discover OD bugs probabilistically.
 */
        ...
        for (int i = 0; i < rounds && !ctx.timeBudgetExhausted(); i++) {
            ...
            for (String failed : result.failedTests()) {
                ...
                findings.add(new ODResult(failed, ODType.VICTIM,
                        List.of("(polluter unknown)", failed),
                        "Failed in random order (round " + (i + 1) + ")",
                        0.7));
            }
        }
