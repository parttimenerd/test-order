...
/**
 * Manages ordering constraints derived from OD detection results.
 * Supports MUST_PRECEDE and MUST_NOT_PRECEDE constraints and produces
 * a topologically valid ordering.
 */
public class OrderConstraintManager {
    ...
    public void applyResults(List<ODResult> results) {
        for (ODResult result : results) {
            if (result.type() == ODType.BRITTLE && result.dependencyChain().size() >= 2) {
                ...
                addMustPrecede(setter, result.victim(),
                        "Brittle: " + result.victim() + " needs " + setter);
            } else if (result.type() == ODType.VICTIM && result.dependencyChain().size() >= 2) {
                String polluter = result.dependencyChain().get(0);
                addMustNotPrecede(polluter, result.victim(),
                        "Victim: " + polluter + " pollutes " + result.victim());
            }
        }
    }

    /**
     * Produce an ordering that respects MUST_PRECEDE constraints via topological sort.
     * Falls back to the reference order when no constraints are violated.
     *
     * @param referenceOrder the original test order to base on
    ...
    public List<String> buildConstrainedOrder(List<String> referenceOrder) {
        ...
        for (Constraint c : constraints) {
            if (c.type != ConstraintType.MUST_PRECEDE) continue;
            if (!inDegree.containsKey(c.testA) || !inDegree.containsKey(c.testB)) continue;
    ...
    private void applyMustNotPrecede(List<String> order) {
        ...
        // Simple greedy: if polluter is immediately before victim, swap with next available
