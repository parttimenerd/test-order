...
/**
 * Algorithm 4: Dependence-Aware Bounded detection.
 * Uses dependency information to bound the search space for each test,
 * only checking potential polluters that share dependencies.
 */
public class DependenceAwareBoundedAlgorithm implements DetectionAlgorithm {
    ...
    public List<ODResult> detect(DetectionContext ctx) {
        ...
        if (ctx.depMap() == null) return findings;
        ...
        for (String victim : ctx.passingTests()) {
            ...
            if (victimDeps == null || victimDeps.isEmpty()) continue;
            ...
            if (!isolation.passed(victim)) continue; // Not a victim, just flaky
            ...
            if (result.passed(victim)) continue; // No pollution

            // ddmin to find minimal polluter set
            List<String> minimal = DeltaDebugging.minimize(candidates, victim, ctx.runner(), 15);

            if (!minimal.isEmpty()) {
                findings.add(new ODResult(victim, ODType.VICTIM,
                        buildChain(minimal, victim),
                        "Dep-aware bounded: found via shared deps " + victimDeps.stream()
