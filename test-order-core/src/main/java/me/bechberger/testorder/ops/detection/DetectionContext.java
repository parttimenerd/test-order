 ...
 *
 * @param graph
 *            conflict graph (may be empty for algorithms that don't use it)
 * @param depMap
 *            raw dependency data (nullable if not available)
 * @param state
 *            run history + failure scores
 * @param referenceOrder
 *            last known passing order
 * @param passingTests
 *            tests that pass in reference order
 * @param runner
 *            executes test orders and reports results
 * @param deadlineMillis
 *            hard stop time (epoch millis)
 * @param randomSeed
 *            for reproducibility
 * @param log
 *            logger for progress messages
 */
public record DetectionContext(ConflictGraph graph, DependencyMap depMap, TestOrderState state,
		List<String> referenceOrder, Set<String> passingTests, TestRunner runner, long deadlineMillis, long randomSeed,
		PluginLog log, AtomicInteger runCounter) {
	...
	public DetectionContext(ConflictGraph graph, DependencyMap depMap, TestOrderState state,
			List<String> referenceOrder, Set<String> passingTests, TestRunner runner, long deadlineMillis,
			long randomSeed) {
		this(graph, depMap, state, referenceOrder, passingTests, runner, deadlineMillis, randomSeed, null,
				new AtomicInteger(0));
	}
	...
	public void recordRun(int findingsSoFar) {
		...
		if (log != null && count % 5 == 0) {
			long remaining = deadlineMillis == Long.MAX_VALUE
					? -1
					: (deadlineMillis - System.currentTimeMillis()) / 1000;
			String budget = remaining < 0 ? "unlimited" : remaining + "s remaining";
			log.info("[progress] " + count + " runs completed, " + findingsSoFar + " findings so far (" + budget + ")");
		}
	}
