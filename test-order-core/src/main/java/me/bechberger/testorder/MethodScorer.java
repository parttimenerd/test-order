 ...
 * <ul>
 *     <li><b>Failure recency:</b> Recent failures boost score so flaky methods run first.</li>
 *     <li><b>Speed relative to class median:</b> Fast/slow compared to other methods in the same class.</li>
 *     <li><b>Dependency overlap:</b> Methods whose per-method deps overlap more with changed classes
 *         get a higher score (available with FULL_METHOD and FULL_MEMBER instrumentation modes).</li>
 *     <li><b>New method:</b> Methods with no telemetry history get a bonus (untested = risky).</li>
 *     <li><b>Changed method:</b> Methods whose source code changed get a bonus.</li>
 * </ul>
 * <p>
 * Key difference from class-level scoring: speed thresholds are <b>class-local</b>.
 * A method's duration is compared against the median of all methods in its class,
 * not a global median. This ensures fair comparison across classes of different speeds.
 */
public class MethodScorer {
    ...
    public record MethodScoreResult(
            String className,
            String methodName,
            double score,
            double failureRecencyBonus,
            double speedBonus,
            double depOverlapBonus,
            double coverageBonus,
            double newMethodBonus,
            double changedMethodBonus,
            boolean isRecent,
            boolean isNew,
            boolean isChanged,
            boolean isFast,
    ...
    /**
     * Creates a method scorer with method-level weights, state, and optional dependency/change data.
     *
     * @param weights          method-level scoring weights
     * @param state            test order state containing failure history
     * @param depMap           dependency map with per-method deps (may be null)
     * @param changedClasses   set of changed class FQCNs (may be null)
     * @param changedMethods   set of changed method keys className#methodName (may be null)
     */
    public MethodScorer(TestOrderState.MethodScoringWeights weights,
     ...
     * <p>
     * Speed bonuses/penalties are calculated relative to the median duration of methods
     * in the same class (passed as classMedianMs). This ensures methods are fairly compared
     * to their peers.
     *
     * @param methods methods to score, all from the same class
     * @return list of scored results (same order as input unless specified otherwise)
     */
    public List<MethodScoreResult> score(List<MethodMetadata> methods) {
        ...
        for (MethodMetadata m : methods) {
            ...
            double score = failRecencyBonus + speedBonus + depOverlapBonus
                    + coverageBonusVal + newMethodBonus + changedMethodBonus;
            ...
            results.add(new MethodScoreResult(
                    m.className(),
                    m.methodName(),
                    score,
                    failRecencyBonus,
    ...
    /**
     * Greedy set-cover for methods: iteratively pick the method whose deps
     * cover the most not-yet-covered changed classes, awarding a declining bonus.
     *
    ...
    private Map<String, Double> computeSetCoverBonuses(List<MethodMetadata> methods, double weight) {
        if (depMap == null || changedClasses.isEmpty() || weight <= 0) return Map.of();
        ...
        for (MethodMetadata m : methods) {
            ...
            if (methodDeps == null || methodDeps.isEmpty()) continue;
            Set<String> covered = new HashSet<>();
            for (String dep : methodDeps) {
                if (changedClasses.contains(dep)) covered.add(dep);
            }
            if (!covered.isEmpty()) {
     ...
     * <p>
     * Uses {@code overlap / sqrt(totalMethodDeps)} normalisation (matching class-level
     * formula) scaled by the depOverlap weight.
     * Returns 0 if no method-level dependency data is available.
     */
    private double computeDepOverlapBonus(MethodMetadata m) {
        ...
        for (String dep : methodDeps) {
            if (changedClasses.contains(dep)) intersectionSize++;
        }
    ...
    /**
     * Computes new method bonus.
     * A method is "new" if it has no duration history in the state (never seen before).
     */
    private double computeNewMethodBonus(MethodMetadata m) {
        if (weights.newMethod() == 0) return 0.0;
    ...
    /**
     * Computes changed method bonus.
     * A method is "changed" if its source hash differs from the previous snapshot.
     */
    private double computeChangedMethodBonus(MethodMetadata m) {
        if (weights.changedMethod() == 0 || changedMethods.isEmpty()) return 0.0;
        String methodKey = m.className() + "#" + m.methodName();
     ...
     * <p>
     * Uses ceil(failScore) capped at failureRecency weight — consistent with class-level
     * formula where each accumulated failure contributes 1 point up to the weight cap.
     * Uses methodFailureScore() which includes both historical and current-run failures.
     */
     ...
     * <p>
     * Maps {@code duration/classMedian} to log₂ space, clamped to [−3, +3].
     * Scores range from {@code +fast} (8× faster) through 0 (at median) to
     * {@code -slow} (8× slower).
     */
    private double computeSpeedBonus(long duration, long classMedian) {
        if (classMedian <= 0 || duration < 0 || (weights.fast() == 0 && weights.slow() == 0)) return 0.0;
    ...
    /**
     * Computes the median duration of methods in a class.
     * Methods with unknown duration (negative values) are excluded.
     */
    private long computeMedianDuration(List<MethodMetadata> methods) {
        ...
        int count = 0;
        for (MethodMetadata m : methods) {
            if (m.durationMs() >= 0) durations[count++] = m.durationMs();
        }
        if (count == 0) return 0;
