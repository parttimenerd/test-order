...
class SetCoverComputerTest {
	...
	class EdgeCases {
		...
		void allTestsIdenticalDepSetsUniformInitialCounts() {
			...
			// they should have uniform initialCoverCounts, but greedy order depends on
			// first selection
			Map<String, Set<String>> coverage = new HashMap<>();
			...
			// Should select one test (others are redundant) since first test covers
			// everything
		...
		void staleQueueEntryCountSkipped() {
			...
			// it should be re-queued with correct count and later skipped when popped with
			// wrong count
			Map<String, Set<String>> coverage = new HashMap<>();
			coverage.put("test1", Set.of("dep1", "dep2", "dep3"));
