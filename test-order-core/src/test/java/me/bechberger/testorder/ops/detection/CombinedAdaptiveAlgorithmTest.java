...
class CombinedAdaptiveAlgorithmTest {
    ...
    /**
     * Simulates a test suite where "Polluter" pollutes state causing "Victim" to fail.
     */
    ...
    void detectsPolluterVictimPair() {
        ...
        DetectionContext ctx = new DetectionContext(
                ConflictGraph.empty(), null, null,
                List.of("Victim", "Polluter", "Other"),
                Set.of("Victim", "Polluter", "Other"),
                polluterRunner(), Long.MAX_VALUE, 42L);
    ...
    void noFindingsForStableTests() {
        TestRunner stableRunner = order -> new TestRunner.TestRunResult(
        ...
        DetectionContext ctx = new DetectionContext(
                ...
                List.of("A", "B", "C"),
                Set.of("A", "B", "C"),
                stableRunner, Long.MAX_VALUE, 42L);
    ...
    void respectsTimeBudget() {
        ...
        DetectionContext ctx = new DetectionContext(
                ...
                List.of("A", "B", "C"),
                Set.of("A", "B", "C"),
                runner, 0L, 42L);  // deadline = 0 → immediately expired
    ...
    void detectsBrittleTest() {
        ...
        TestRunner runner = order -> {
            ...
            for (String test : order) {
                if (test.equals("Setter")) {
                    ...
                    if (setup) passed.add(test); else failed.add(test);
                } else {
                    passed.add(test);
                }
            }
        ...
        DetectionContext ctx = new DetectionContext(
                ConflictGraph.empty(), null, null,
                List.of("Setter", "Brittle", "Other"),
                Set.of("Setter", "Brittle", "Other"),
