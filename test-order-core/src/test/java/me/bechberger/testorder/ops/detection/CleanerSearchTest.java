...
class CleanerSearchTest {
	...
	void findsCleanerThatNeutralizesPolluterEffect() {
		...
		Optional<String> cleaner = CleanerSearch.find("Polluter", "Victim", List.of("A", "Cleaner", "B"), runner, 10);
	...
	void returnsEmptyWhenNoCleanerExists() {
		...
		Optional<String> cleaner = CleanerSearch.find("Polluter", "Victim", List.of("A", "B", "C"), runner, 10);
	...
	void returnsEmptyWhenBaselineDoesNotFail() {
		...
		Optional<String> cleaner = CleanerSearch.find("Polluter", "Victim", List.of("A", "B"), runner, 10);
	...
	void findAllReturnsMultipleCleaners() {
		...
		List<String> cleaners = CleanerSearch.findAll("Polluter", "Victim", List.of("A", "Clean1", "B", "Clean2"),
				runner, 10);
	...
	void respectsRunBudget() {
		...
		TestRunner runner = order -> {
			...
			for (String t : order) {
				if (t.equals("Victim") && order.contains("Polluter")
						&& order.indexOf("Polluter") < order.indexOf("Victim") && !order.contains("Cleaner")) {
					failed.add(t);
				} else {
					passed.add(t);
				}
		...
		CleanerSearch.find("Polluter", "Victim", List.of("A", "B", "C", "D", "E"), runner, 2);
