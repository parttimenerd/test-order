...
class CoverageOperationTest {
	...
	void analyzeExcludesTestClassesFromProductionEntries() {
		DependencyMap depMap = new DependencyMap();
		// FooTest depends on itself (e.g. a test that calls static methods on itself)
		// AND on Foo
		depMap.put("com.example.FooTest", Set.of("com.example.Foo", "com.example.FooTest"));

		CoverageAnalysis result = CoverageOperation.analyze(depMap, NO_OP_LOG);

		// FooTest is a test class — it must not appear as a production class in
		// coverage
		...
		boolean hasFoo = result.entries().stream().anyMatch(c -> c.fullyQualifiedName().equals("com.example.Foo"));
	...
	void noMemberCoveragePercentWhenAllMembersUnknown() {
		...
		var fooEntry = result.entries().stream().filter(c -> c.fullyQualifiedName().equals("com.example.Foo"))
				.findFirst();
	...
	void classCoverageConstructorWithAllMembersKnown() {
		...
		ClassCoverage c = new ClassCoverage("com.example.Foo", List.of("FooTest"),
				Set.of("method1", "method2", "method3"), Set.of("method1", "method2"),
