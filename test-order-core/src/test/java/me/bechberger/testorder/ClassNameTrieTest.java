...
class ClassNameTrieTest {
	...
	void serializeDeserializeRoundTrip() throws IOException {
		ClassNameTrie trie = new ClassNameTrie();
		List<String> names = List.of("org.springframework.samples.petclinic.owner.OwnerControllerTest",
				"org.springframework.samples.petclinic.owner.OwnerController",
				"org.springframework.samples.petclinic.vet.VetController",
				"org.springframework.samples.petclinic.vet.VetControllerTest", "com.example.Simple");
		for (String n : names) {
			trie.insert(n);
		}
		trie.assignIds();
		...
		ClassNameTrie loaded = ClassNameTrie.readFrom(new DataInputStream(new ByteArrayInputStream(bos.toByteArray())));
