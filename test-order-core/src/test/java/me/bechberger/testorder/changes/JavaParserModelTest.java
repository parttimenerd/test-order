...
/**
 * Tests that the JavaParser mode produces equivalent results to the
 * island-grammar parser for core structural parsing scenarios (types, methods,
 * fields, initializers).
 * <p>
...
class JavaParserModelTest {
	...
	void structuralDiffMethodAdded() {
		...
		var added = diff.changes().stream().filter(c -> c.kind() == StructuralDiff.Change.Kind.ADDED).toList();
	...
	void structuralDiffFieldAdded() {
		...
		StructuralDiff.FileDiff diff = StructuralDiff.diffSources(Path.of("Foo.java"), oldSource, newSource);
		assertTrue(diff.hasChanges());
		var added = diff.changes().stream().filter(c -> c.category() == StructuralDiff.Change.Category.FIELD).toList();
		assertEquals(1, added.size());
	...
	void methodNamesMatchIslandMode() {
		...
		source = """
				...
				""";
		SourceFileModel.setParserMode(SourceFileModel.ParserMode.JAVAPARSER);
		var jpMethods = SourceFileModel.parse(source, "p", SourceFileModel.Detail.METHODS).methods().stream()
				.map(SourceFileModel.MethodNode::name).sorted().toList();
		SourceFileModel.setParserMode(SourceFileModel.ParserMode.ISLAND);
		var islandMethods = SourceFileModel.parse(source, "p", SourceFileModel.Detail.METHODS).methods().stream()
				.map(SourceFileModel.MethodNode::name).sorted().toList();
	...
	void fieldNamesMatchIslandMode() {
		String source = """
				...
				""";
		SourceFileModel.setParserMode(SourceFileModel.ParserMode.JAVAPARSER);
		var jpFields = SourceFileModel.parse(source, "p", SourceFileModel.Detail.FIELDS).fields().stream()
				.map(SourceFileModel.FieldNode::name).sorted().toList();
		SourceFileModel.setParserMode(SourceFileModel.ParserMode.ISLAND);
		var islandFields = SourceFileModel.parse(source, "p", SourceFileModel.Detail.FIELDS).fields().stream()
				.map(SourceFileModel.FieldNode::name).sorted().toList();
