...
/**
 * Mojo goal: coverage Analyzes test coverage across the project and identifies
 * least tested classes. Generates markdown reports and JSON output for CI
 * integration.
 */
...
public class CoverageMojo extends AbstractMojo {
	...
	/**
	 * Output format for coverage reports. Valid values: 'comprehensive' (default,
	...
	/**
	 * Comma-separated list of module names to include in analysis. If not set, all
	 * modules are included. Example: '-DincludeModules=core,cli'
	 */
	...
	public void execute() throws MojoExecutionException, MojoFailureException {
		...
		try {
			...
			for (MavenProject project : reactorProjects) {
				...
				if (jacocoXml.exists()) {
					...
					for (ClassMetrics metric : metrics) {
						ClassMetrics updated = new ClassMetrics(metric.getFullyQualifiedName(), moduleName,
								metric.getPackageName(), metric.getClassName(), metric.getLineCoverage(),
								metric.getMethodCoverage(), metric.getBranchCoverage(), metric.getStatementsCovered(),
								metric.getStatementsTotal(), metric.getMethodsCovered(), metric.getMethodsTotal(),
								metric.getTestCount(), metric.getTestNames(), metric.isAbstract(), metric.isInterface(),
								metric.isEnum());
			...
			for (ClassMetrics metric : allMetrics) {
				...
				ClassMetrics merged = new ClassMetrics(metric.getFullyQualifiedName(), metric.getModule(),
						metric.getPackageName(), metric.getClassName(), metric.getLineCoverage(),
						metric.getMethodCoverage(), metric.getBranchCoverage(), metric.getStatementsCovered(),
						metric.getStatementsTotal(), metric.getMethodsCovered(), metric.getMethodsTotal(), testCount,
						metric.getTestNames(), metric.isAbstract(), metric.isInterface(), metric.isEnum());
