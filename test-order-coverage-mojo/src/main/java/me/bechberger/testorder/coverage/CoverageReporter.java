...
public class CoverageReporter {
	...
	public String generateDetailedReport(int threshold) {
		...
		Map<LeastTestedClassifier.Severity, List<ClassMetrics>> classified = classifier.classifyByThreshold(threshold);
		...
		for (LeastTestedClassifier.Severity severity : LeastTestedClassifier.Severity.values()) {
			...
			sb.append("\n## ").append(severity.getLabel()).append(" (").append(classes.size()).append(" classes)\n\n");

			for (ClassMetrics metric : classes) {
				sb.append(String.format("- `%s` (%d%%) [Module: %s] Tests: %d\n", metric.getFullyQualifiedName(),
						metric.getLineCoverage(), metric.getModule(), metric.getTestCount()));
			}
		}
	...
	public String generateModuleReport() {
		...
		for (Map.Entry<String, CoverageReport.ModuleStats> entry : moduleStats.entrySet()) {
			...
			sb.append(String.format("| %s | %d%% | %d | %d | %d | %d |\n", entry.getKey(), (int) avgCov,
					stats.getClassCount(), stats.getHighCoverageCount(80), stats.getMediumCoverageCount(50, 80),
		...
		if (moduleCount > 0) {
			...
			sb.append(String.format("| **Overall** | **%d%%** | **%d** | **%d** | **%d** | **%d** |\n",
					(int) (totalAvg / moduleCount), overall.getClassCount(), 0, // placeholder
					0, // placeholder
					0)); // placeholder
		}
	...
	public String generateRecommendations(int threshold) {
		...
		List<ClassMetrics> critical = classified.getOrDefault(LeastTestedClassifier.Severity.CRITICAL,
				new ArrayList<>());
		if (!critical.isEmpty()) {
		...
		List<ClassMetrics> important = classified.getOrDefault(LeastTestedClassifier.Severity.IMPORTANT,
				new ArrayList<>());
