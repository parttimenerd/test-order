...
public class LeastTestedClassifierTest {
	...
	void testClassifyBySeverity() {
		Map<LeastTestedClassifier.Severity, List<ClassMetrics>> classified = classifier.classifyByThreshold(50);
	...
	void testSortingByCoverage() {
		...
		for (int i = 0; i < leastTested.size() - 1; i++) {
			assertTrue(leastTested.get(i).getLineCoverage() <= leastTested.get(i + 1).getLineCoverage());
		}
	}
}
