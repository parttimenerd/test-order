...
class BugVerificationIT {
	...
	void showOrderWithoutIndexShouldNotCreateEmptyIndex() {
		...
		if (result.isSuccess()) {
			...
			if (depMap != null) {
				...
				assertThat(depMap.size()).as("show-order should not create an empty index with 0 test classes")
						.isGreaterThan(0);
			}
		}
		// If it failed, that's also acceptable — at least it didn't silently corrupt
		// state
	}
	...
	void runRemainingWithEmptyFileShouldNotRunAllTests() {
		...
		// Select — with all source classes changed, all 3 tests should be selected
		MavenResult select = project.maven().select(PRODUCT, CART, INVOICE);
		...
		assertThat(remaining == null || remaining.isBlank())
				.as("All tests should have been selected (remaining file should be empty/absent)").isTrue();
		...
		if (runRemaining.isSuccess()) {
			...
			int totalTestsRun = testRunLines.stream().mapToInt(line -> {
	...
	void nonExistentWeightsFileShouldWarnOrFail() {
		...
		MavenResult result = project.maven().run("test-order:show-order", "-Dtestorder.changeMode=explicit",
		...
		if (result.isSuccess()) {
			assertThat(result.output()).as("Non-existent weights file should produce a warning in output")
	...
	void systemPropertyScoreOverridesShouldWork() {
		...
		MavenResult noOverlap = project.maven().run("test-order:show-order", "-Dtestorder.changeMode=explicit",
				"-Dtestorder.changed.classes=" + PRODUCT, "-Dtestorder.score.depOverlap=0");
		...
		MavenResult highOverlap = project.maven().run("test-order:show-order", "-Dtestorder.changeMode=explicit",
				"-Dtestorder.changed.classes=" + PRODUCT, "-Dtestorder.score.depOverlap=100");
	...
	void weightsFileOverridesDoWork() {
		...
		try {
			...
			MavenResult result = project.maven().run("test-order:show-order", "-Dtestorder.changeMode=explicit",
					"-Dtestorder.changed.classes=" + PRODUCT, "-Dtestorder.weights.file=test-weights.txt");
			assertThat(result).succeeded();

			int productScore = extractScore(result.output(), "ProductTest");
			// With depOverlap=100 and all other bonuses=0, ProductTest should have a high
			// score
			assertThat(productScore).as("Weights file with depOverlap=100 should produce a high score")
	...
	void orderModeRunsAffectedTestsFirst() {
		...
		assertThat(cartPos).as("CartTest should appear before ProductTest when Cart is changed").isLessThan(productPos);
	}
	...
	void showOrderMultipleChangedClasses() {
		...
		assertThat(invoiceScore).as("InvoiceTest should score > ProductTest when Cart+Invoice changed")
	...
	void failingTestRecordedInState() {
		...
		try {
			...
			project.replaceInFile("src/main/java/com/example/shop/Cart.java", "return items.size();",
					"return items.size() + 1;");
	...
	void combinedModeFromScratchCreatesValidIndex() {
		...
		DependencyMap depMap = project.loadIndex();
		assertThat(depMap).isLoaded().hasSize(3).hasTestClass(PRODUCT_TEST).hasTestClass(CART_TEST)
	...
	void selectWithEmptySelectionShouldNotRunAllTests() {
		...
		if (result.isSuccess()) {
			...
			assertThat(totalTestsRun).as("select with topN=0, randomM=0 should run 0 tests, not %d", totalTestsRun)
					.isEqualTo(0);
		}
	...
	void combinedModeShouldInformAboutRemainingTests() {
		...
		assertThat(result.output()).as("Combined mode should tell user to run: mvn test-order:run-remaining test")
	...
	void aggregateWithEmptyDepsShouldNotCreateEmptyIndex() {
		...
		assertThat(project.exists(".test-order/test-dependencies.lz4"))
				.as("aggregate with empty deps dir should not create an index file").isFalse();

		// Should have warned
		assertThat(result.output()).as("aggregate with empty deps should produce a warning")
				.containsIgnoringCase("warn");
	}
	...
	void selectShouldUseFqcnsInTestFilter() {
		// Always start from clean state to avoid batch contamination
		project.cleanAll();
		MavenResult learn = project.maven().learn();
		assertThat(learn).succeeded();
		...
		assertThat(selected.lines().toList()).as("Selected file should contain fully-qualified class names")
	...
	void learnModeShouldNotDoubleRecordFailures() {
		...
		try {
			...
			// With exponential decay model, we verify score is present (count no longer
			// tracked).
			assertThat(state).hasFailureFor(CART_TEST);
		} finally {
			project.restoreAll();
	...
	void learnModeShouldCreateStateFile() {
		...
		var state = project.loadState();
		assertThat(state).isLoaded().hasDuration(PRODUCT_TEST).hasDuration(CART_TEST).hasDuration(INVOICE_TEST);
	}
	...
	void runRemainingWithMissingFileShouldSkip() {
		...
		assertThat(remaining).succeeded().outputContains("nothing to run").outputContains("Tests are skipped");
	}
	...
	void optimizeShouldWorkWithReactorContext() {
		...
		project.replaceInFile("src/test/java/com/example/shop/ProductTest.java",
				"assertEquals(\"Widget\", p.getName());", "assertEquals(\"WRONG\", p.getName());");
		...
		assertThat(optimize).succeeded().outputContains("[test-order]");
	}
	...
	void configPathsShouldBeCorrectlyEscaped() throws Exception {
		...
		java.nio.file.Path configFile = project.getProjectDir()
				.resolve("target/test-order-runtime/testorder-config.properties");
	...
	void orderModeSinceLastCommitOnSingleCommitRepoSucceeds(
			@org.junit.jupiter.api.io.TempDir java.nio.file.Path tempRepo) throws Exception {
		...
		TestProject shallowProject = new TestProject(tempRepo, List.of("-Dtestorder.includePackages=com.example"));
		...
		MavenResult order = shallowProject.maven().run("clean", "test", "-Dtestorder.mode=order",
	...
	//
	// Steps: learn (creates state), write garbage to .test-order/state.lz4,
	// run order mode.
	...
	// -Dtestorder.mode=order on CLI → plugin ignores CLI and runs
	// learn mode. Expected: CLI wins.
	// ═══════════════════════════════════════════════════════════════════

	@Test
	@Order(240)
	@DisplayName("BUG: CLI -Dtestorder.mode=order overrides POM-level <mode>learn</mode>")
	void cliModeOverridesPomLevelMode() {
		// Ensure we have learned data
		if (!project.exists(".test-order/test-dependencies.lz4")) {
			project.cleanAll();
			project.maven().learn();
		}
		...
		try {
			...
			assertThat(result.output()).doesNotContain("Learn mode");
		} finally {
			project.restoreAll();
		}
	}
