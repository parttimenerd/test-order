...
class SystematicBugHuntIT {
	...
	class LearnModeTests {
		...
		void shouldCreateIndexWithZeroChanges() throws Exception {
			...
			assertThat(Files.exists(indexPath)).as("Index file should exist after learn mode").isTrue();
		}
	...
	class OrderModeTests {
		...
		void shouldHandleMissingIndex() throws Exception {
			...
			assertThat(result).as("Should output BUILD result").outputContains("BUILD");
		}
		...
		void shouldDetectChangedSources() throws Exception {
			...
			project.replaceInFile("src/main/java/com/myapp/service/MathService.java", "return a + b;",
					"return a + b + 0; // BUG");

			// Order mode should detect the change
			var orderResult = project.maven().auto();
			assertThat(orderResult).outputContains("[test-order]");
		}
	}
	...
	class SelectModeTests {
		...
		void shouldCreateSelectionFiles() throws Exception {
			...
			assertThat(Files.exists(selectedFile)).as("Selected tests file should exist").isTrue();
			assertThat(Files.exists(remainingFile)).as("Remaining tests file should exist").isTrue();
		}
		...
		void shouldIncludeNewTests() throws Exception {
			...
			assertThat(selected).as("New test should be included in selection").contains("NewTest");
		}
	}
	...
	class ErrorHandlingTests {
		...
		void shouldHandleCorruptedState() throws Exception {
			...
			var result = project.maven().order();
			assertThat(result).as("Should handle corrupted state file gracefully").outputContains("BUILD");
		}
	...
	class DashboardTests {
		...
		void shouldGenerateDashboard() throws Exception {
			...
			var dashboardPath = project.getProjectDir().resolve("target/test-order-dashboard/index.html");
			assertThat(Files.exists(dashboardPath)).as("Dashboard HTML should be generated").isTrue();
		}
	...
	class DocumentationTests {
		...
		void shouldDisplayHelp() throws Exception {
			...
			assertThat(result).succeeded().outputContains("Goal");
		}
