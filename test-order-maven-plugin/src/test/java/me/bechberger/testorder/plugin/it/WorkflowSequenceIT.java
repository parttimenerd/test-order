...
class WorkflowSequenceIT {
	...
	@Test
	@Order(40)
	@DisplayName("Duration tracking accumulates across multiple order runs")
	void durationTrackingAccumulates() {
		shopProject.cleanAll();
		shopProject.maven().learn();

		// First order run
		shopProject.maven().order(PRODUCT);
		TestOrderState state1 = shopProject.loadState();
		long dur1 = state1 != null ? state1.getDuration(PRODUCT_TEST, -1) : -1;

		// Second order run
		shopProject.maven().order(PRODUCT);
		TestOrderState state2 = shopProject.loadState();
		long dur2 = state2 != null ? state2.getDuration(PRODUCT_TEST, -1) : -1;

		// Duration should exist after at least one run
		assertThat(dur1 >= 0 || dur2 >= 0)
				.as("Duration should be tracked after order runs")
				.isTrue();
	}
