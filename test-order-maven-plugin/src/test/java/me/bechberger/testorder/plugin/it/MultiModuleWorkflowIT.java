...
class MultiModuleWorkflowIT {
	...
	void perModuleStateFilesIndependent() {
		// After order run, state should exist for modules that have tests
		// State files are per-module
		boolean coreState = multiProject.exists("core/.test-order/state.lz4");
		boolean webState = multiProject.exists("web/.test-order/state.lz4");

		// At least one module should have a state file
		assertThat(coreState || webState)
	...
	void relearnAfterCoreModuleSourceChange() {
		// Modify UserService to add a new method
		multiProject.appendToFile(
