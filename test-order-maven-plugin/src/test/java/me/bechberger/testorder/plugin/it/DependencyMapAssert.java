 ...
 * <p>
 * Usage:
 * {@code assertThat(depMap).hasTestClass("com.example.CalculatorTest").hasDependency("com.example.CalculatorTest", "com.example.Calculator")}
 */
public class DependencyMapAssert extends AbstractAssert<DependencyMapAssert, DependencyMap> {
	...
	public DependencyMapAssert hasSize(int expected) {
		...
		if (actual.size() != expected) {
			failWithMessage("Expected %d test classes but found %d: %s", expected, actual.size(), actual.testClasses());
		}
		return this;
	}
	...
	public DependencyMapAssert hasTestClass(String testClass) {
		...
		if (!actual.testClasses().contains(testClass)) {
			failWithMessage("Expected test class '%s' in index but found: %s", testClass, actual.testClasses());
		}
		return this;
	}
	...
	public DependencyMapAssert hasDependency(String testClass, String dependency) {
		...
		if (deps == null || !deps.contains(dependency)) {
			failWithMessage("Expected '%s' to depend on '%s' but its deps are: %s", testClass, dependency, deps);
		}
		return this;
	}
	...
	public DependencyMapAssert doesNotHaveDependency(String testClass, String dependency) {
		...
		Set<String> deps = actual.get(testClass);
		if (deps != null && deps.contains(dependency)) {
			failWithMessage("Expected '%s' to NOT depend on '%s' but it does. Deps: %s", testClass, dependency, deps);
		}
		return this;
	}
	...
	public DependencyMapAssert hasAtLeastDeps(String testClass, int minDeps) {
		...
		if (count < minDeps) {
			failWithMessage("Expected '%s' to have at least %d deps but found %d: %s", testClass, minDeps, count, deps);
		}
		return this;
	}
	...
	public DependencyMapAssert changesAffect(Set<String> changedClasses, String testClass) {
		...
		if (!affected.contains(testClass)) {
			failWithMessage("Expected change to %s to affect '%s' but affected tests are: %s", changedClasses,
					testClass, affected);
		}
		return this;
	}
	...
	public DependencyMapAssert changesDoNotAffect(Set<String> changedClasses, String testClass) {
		...
		Set<String> affected = actual.getAffectedTests(changedClasses);
		if (affected.contains(testClass)) {
			failWithMessage("Expected change to %s to NOT affect '%s' but it did. Affected: %s", changedClasses,
					testClass, affected);
		}
		return this;
	}
