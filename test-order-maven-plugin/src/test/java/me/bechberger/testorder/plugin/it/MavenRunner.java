...
/**
 * Runs Maven commands against a project directory, capturing output.
 * Discovers Maven from PATH or MAVEN_HOME.
 */
public class MavenRunner {
    ...
    /** Run {@code mvn clean test -Dtestorder.mode=order} with explicit changed classes */
    public MavenResult order(String... changedClasses) {
        List<String> args = new ArrayList<>(List.of("clean", "test", "-Dtestorder.mode=order",
    ...
    /** Run {@code mvn clean test} (auto mode — the default) with explicit changed classes */
    public MavenResult auto(String... changedClasses) {
        List<String> args = new ArrayList<>(List.of("clean", "test",
                "-Dtestorder.changeMode=explicit"));
        if (changedClasses.length > 0) {
            args.add("-Dtestorder.changed.classes=" + String.join(",", changedClasses));
        }
    ...
    public MavenResult showOrder(String... changedClasses) {
        List<String> args = new ArrayList<>(List.of("test-order:show-order",
                "-Dtestorder.changeMode=explicit"));
        if (changedClasses.length > 0) {
            args.add("-Dtestorder.changed.classes=" + String.join(",", changedClasses));
        }
    ...
    private static String findMavenExecutable() {
        ...
        if (mavenHome != null) {
            ...
            if (mvn.toFile().canExecute()) return mvn.toString();
        }
