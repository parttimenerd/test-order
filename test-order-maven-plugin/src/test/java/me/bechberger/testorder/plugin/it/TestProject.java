 ...
 * <p>
 * Provides helpers to run Maven goals, read internal files,
 * modify source files, and restore them after tests.
 */
public class TestProject {
        ...
        try {
            ...
            if (!content.contains(oldText)) {
                throw new IllegalArgumentException(
                        "File " + relativePath + " does not contain: " + oldText);
            }
    ...
    public TestProject cleanAll() {
        deleteIfExists("test-dependencies.lz4");
        deleteIfExists(".test-order-state");
        deleteIfExists(".test-order-hashes.lz4");
        deleteIfExists(".test-order-test-hashes.lz4");
        deleteTree("target");
        return this;
    }
    ...
    /** Load the dependency index (test-dependencies.lz4). Returns null if it doesn't exist. */
    public DependencyMap loadIndex() {
        return loadIndex("test-dependencies.lz4");
    }
    ...
    public TestOrderState loadState() {
        return loadState(".test-order-state");
    }
