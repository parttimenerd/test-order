...
/**
 * Abstract base for end-to-end integration tests against the test-order Maven plugin.
 * <p>
 * Each subclass specifies a target project via {@link #projectDirName()}.
 * The target project must contain:
 * <ul>
 *   <li>{@code CalculatorTest} → depends on {@code Calculator}, {@code MathHelper}</li>
 *   <li>{@code StringUtilsTest} → depends on {@code StringUtils}</li>
 * </ul>
 * <p>
 * Tests are ordered so that learn mode runs first (builds the index),
 * then subsequent tests use that index.
 * <p>
...
abstract class AbstractEndToEndIT {
    ...
    void setup() {
        ...
        project = new TestProject(root.resolve(projectDirName()),
    ...
    void learnModeProducesIndex() {
        ...
        assertThat(depMap).isLoaded()
                .hasSize(2)
                .hasTestClass(CALCULATOR_TEST)
                .hasTestClass(STRING_UTILS_TEST);

        // Verify dependency relationships
        assertThat(depMap)
                .hasDependency(CALCULATOR_TEST, CALCULATOR)
    ...
    void affectedTestsCalculator() {
        ...
        assertThat(depMap)
                .changesAffect(Set.of(CALCULATOR), CALCULATOR_TEST)
                .changesDoNotAffect(Set.of(CALCULATOR), STRING_UTILS_TEST);
    }
    ...
    void affectedTestsStringUtils() {
        ...
        assertThat(depMap)
                .changesAffect(Set.of(STRING_UTILS), STRING_UTILS_TEST)
    ...
    void affectedTestsMathHelper() {
        ...
        assertThat(depMap)
                .changesAffect(Set.of(MATH_HELPER), CALCULATOR_TEST)
                .changesDoNotAffect(Set.of(MATH_HELPER), STRING_UTILS_TEST);
    }
    ...
    void orderModeWritesJunitPlatformProperties() {
        ...
        assertThat(props).isNotNull()
                .contains("me.bechberger.testorder.PriorityClassOrderer");
    }
    ...
    void showOrderDisplaysScores() {
        ...
        assertThat(result).succeeded()
                .outputContains("CalculatorTest")
                .outputContains("StringUtilsTest");
    }
    ...
    void dumpShowsIndex() {
        ...
        assertThat(result).succeeded()
                .outputContains(CALCULATOR_TEST)
                .outputContains(CALCULATOR);
    }
    ...
    void hashSnapshotsExist() {
        ...
        assertThat(project.exists(".test-order-hashes.lz4") || project.exists(".test-order-test-hashes.lz4"))
                .as("At least one hash snapshot file should exist after order-mode runs")
                .isTrue();
    }
    ...
    void verboseModeWritesLogFile() {
        ...
        assertThat(log).isNotNull()
                .contains("[test-order] Verbose logging enabled")
                .contains("[transform] OK:")
                .contains("[flush]");
    }
    ...
    void relearnAfterModification() {
        try {
            ...
            project.replaceInFile(CALCULATOR_SRC,
