...
class PluginInteractionIT {
	...
	@Test
	@Order(30)
	@DisplayName("Select mode with surefire includes filter works correctly")
	void selectModeWithSurefireIncludes() {
		...
		// Use -Dtestorder.mode=skip to prevent prepare mojo from conflicting
	...
	void learnModeWithCustomArgLine() {
		shopProject.cleanAll();
		MavenResult result = shopProject.maven().run("clean", "test",
				"-Dtestorder.mode=learn",
		...
		// Index should still be produced (argLine chaining worked)
	...
	@Test
	@Order(50)
	@DisplayName("Learn mode with parameterized tests captures class-level deps")
	void learnModeWithParameterizedTests() {
		parameterizedFixture.cleanAll();
		MavenResult result = parameterizedFixture.maven().learn();
		assertThat(result).succeeded();

		DependencyMap depMap = parameterizedFixture.loadIndex();
		assertThat(depMap).isLoaded();
		// Parameterized tests should be recorded at class level
		assertThat(depMap.size()).isGreaterThan(0);
	}

	@Test
	@Order(51)
	@DisplayName("Order mode with parameterized tests works (class ordering applied)")
	void orderModeWithParameterizedTests() {
		...
		// PriorityClassOrderer should be injected
		assertThat(result.output()).contains("PriorityClassOrderer");
	}
	...
	void showOrderUsesStateForDurationScoring() {
		...
		shopProject.maven().order("com.example.shop.Product");
