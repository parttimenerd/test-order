...
/**
 * End-to-end workflow tests against test-order-fields-methods-example.
 * Focuses on member-level dependency capture and since-last-run snapshot behavior.
 * <p>
...
class FieldsMethodsWorkflowIT {
    ...
    private static final String WIDE_COVERAGE_SERVICE_SRC =
            "src/main/java/com/example/coverage/WideCoverageService.java";
    private static final String STATE_MACHINE_SRC =
            "src/main/java/com/example/fields/StateMachine.java";
    ...
    void setup() {
        ...
        project = new TestProject(root.resolve("test-order-fields-methods-example"),
    ...
    @DisplayName("Learn FULL_MEMBER: captures member-level dependencies in the binary index")
        ...
        assertThat(depMap).isLoaded().hasSize(10);
        org.assertj.core.api.Assertions.assertThat(depMap.hasMemberDeps())
                .as("FULL_MEMBER learn mode should persist member-level dependencies")
                .isTrue();
        ...
        org.assertj.core.api.Assertions.assertThat(itemMembers)
                .contains(WIDE_COVERAGE_SERVICE + "#addItem")
                .contains(WIDE_COVERAGE_SERVICE + "#getItemCount");
        org.assertj.core.api.Assertions.assertThat(metadataMembers)
                .contains(WIDE_COVERAGE_SERVICE + "#setMetadata")
                .contains(WIDE_COVERAGE_SERVICE + "#getMetadata");
        org.assertj.core.api.Assertions.assertThat(metadataMembers)
                .doesNotContain(WIDE_COVERAGE_SERVICE + "#addItem");
    }
    ...
    void showOrderDisplaysScoringAfterSourceChange() {
        ...
        // Modify addItem method body so since-last-run detects WideCoverageService as changed
        project.replaceInFile(
                WIDE_COVERAGE_SERVICE_SRC,
                "        accessCount++;\n",
                "        accessCount += 2;\n");

        try {
            MavenResult showOrder = project.maven().run(
            ...
            org.assertj.core.api.Assertions.assertThat(showOrder.output())
                    .contains("ItemMethodsTest")
                    .contains("MetadataMethodsTest")
                    .contains("CombinedMethodsTest");
    ...
    void sinceLastRunIgnoresTimestampOnlyTouch() throws Exception {
        ...
        try {
            ...
            MavenResult showOrder = project.maven().run(
                    ...
                    "-Dtestorder.changeMode=since-last-run");
            assertThat(showOrder).succeeded()
                    .outputContains("StateMachineTest");
            org.assertj.core.api.Assertions.assertThat(showOrder.output())
        ...
        private static int extractDisplayedScore(String output, String testSimpleName) {
                return output.lines()
                                .filter(line -> line.contains(testSimpleName))
