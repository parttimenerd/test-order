 ...
 * sample-shop project (Product → Cart → Invoice dependency chain, 3 test classes).
 * <p>
 * Dependency graph:
 * <pre>
 *   ProductTest  → Product
 ...
 *   learn → inspect index → order → show-order → dump → modify code → re-learn →
 *   select → combined → custom weights → state persistence
 * <p>
...
class UserPerspectiveIT {
    ...
    void productTestDependencies() {
        DependencyMap depMap = project.loadIndex();
        assertThat(depMap)
                .hasDependency(PRODUCT_TEST, PRODUCT)
                .doesNotHaveDependency(PRODUCT_TEST, CART)
    ...
    void cartTestDependencies() {
        DependencyMap depMap = project.loadIndex();
        assertThat(depMap)
                .hasDependency(CART_TEST, CART)
                .hasDependency(CART_TEST, PRODUCT)
    ...
    void invoiceTestDependencies() {
        ...
        assertThat(depMap)
                ...
                .hasDependency(INVOICE_TEST, CART)
    ...
    void changingInvoiceAffectsOnlyInvoiceTest() {
        DependencyMap depMap = project.loadIndex();
        assertThat(depMap)
                .changesAffect(Set.of(INVOICE), INVOICE_TEST)
                .changesDoNotAffect(Set.of(INVOICE), PRODUCT_TEST)
                .changesDoNotAffect(Set.of(INVOICE), CART_TEST);
    }
    ...
    void showOrderDisplaysScores() {
        ...
        assertThat(result).succeeded()
                .outputContains("ProductTest")
                .outputContains("CartTest")
    ...
    void productChangeScoresProductTestHighest() {
        ...
        // All three tests depend on Product, but ProductTest has the highest overlap ratio (1/1)
        ...
        assertThat(productScore)
                .as("ProductTest should score >= CartTest when Product changed")
                .isGreaterThanOrEqualTo(cartScore);
        assertThat(productScore)
                .as("ProductTest should score >= InvoiceTest when Product changed")
    ...
    void invoiceChangeScoresInvoiceTestOnly() {
        ...
        assertThat(invoiceScore)
                .as("InvoiceTest should score > ProductTest when Invoice changed")
    ...
    @Test
    @Order(40)
    @DisplayName("State: order-mode run creates .test-order-state with durations")
    ...
    void secondRunAccumulatesState() {
        ...
        assertThat(state.runs().size())
                .as("Should accumulate run records")
    ...
    void relearnAfterSourceChange() {
        try {
            ...
            project.replaceInFile(PRODUCT_SRC,
    ...
    void selectModeRunsSubset() {
        ...
        assertThat(project.exists("target/test-order-selected.txt")
                || project.exists("target/test-order-remaining.txt"))
                .as("Select mode should produce selection files")
                .isTrue();
    }
    ...
    void customWeightViaSystemProperty() {
        ...
        MavenResult noOverlapResult = project.maven().run(
        ...
        // (it might still differ via speed/failure components, but overlap should be gone)
    ...
    void modifiedTestSourceGetsBonus() {
        ...
        try {
            ...
            MavenResult r1 = project.maven().run("clean", "test",
