...
/**
 * Integration test that runs detect-dependencies against a sample project
 * with known OD bugs (sample-od-bugs).
 *
 * <p>Run directly: {@code java DetectDependenciesIT.java}
 * or via Maven: mvn test -Dtest=DetectDependenciesIT -Dtestorder.it=true
 */
public class DetectDependenciesIT {
    ...
    private static final List<String> TEST_CLASSES = List.of(
            "com.example.od.SetupTest",
            "com.example.od.VictimTest",
            "com.example.od.CounterTest",
            "com.example.od.IndependentTest"
    );
    ...
    private static void runDetection(String algorithm, TestRunner runner) throws IOException {
        ...
        PluginLog log = new PluginLog() {
            ...
            @Override public void debug(String msg) { /* quiet */ }
        };

        Config config = new Config(
                null,  // no dependency index
                null,  // no state file
                outputDir,
                algorithm,
                60,    // 60s time budget
        ...
        System.out.println("  Reference run: passed=" + refResult.passedTests()
                + " failed=" + refResult.failedTests());
        ...
        System.out.println("  Reversed run: passed=" + revResult.passedTests()
                + " failed=" + revResult.failedTests());
        ...
        depMap.putMemberDeps("com.example.od.VictimTest",
                Set.of("com.example.od.GlobalRegistry#instance",
                        "com.example.od.GlobalRegistry#initialized", "com.example.od.GlobalRegistry#lastUser"));
        depMap.putMemberDeps("com.example.od.CounterTest",
                Set.of("com.example.od.GlobalRegistry#instance", "com.example.od.GlobalRegistry#counter"));
        depMap.putMemberDeps("com.example.od.IndependentTest",
                Set.of("com.example.od.GlobalRegistry#instance"));
        ...
        Config configWithDeps = new Config(
                depMapFile, null, outputDir,
                algorithm, 60, false, 42L, "sample-od-bugs", log);
        ...
        try {
            ...
            for (var finding : result.results()) {
                System.out.println("    - " + finding.type() + ": " + finding.victim()
                        + " (confidence=" + String.format("%.0f%%", finding.confidence() * 100) + ")"
                        + " chain=" + finding.dependencyChain());
            }
    ...
    static class SampleProjectRunner implements TestRunner {
        ...
        public TestRunResult run(List<String> testOrder) {
            ...
            String testList = runAll ? null : testOrder.stream()
