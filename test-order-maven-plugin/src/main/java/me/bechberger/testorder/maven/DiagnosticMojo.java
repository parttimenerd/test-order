...
/**
 * Maven goal to diagnose test-order setup and detect common issues. Validates
 * index, state, hash files, permissions, and change detection.
 *
 * Usage: mvn test-order:diagnose
 */
@Mojo(name = "diagnose", defaultPhase = LifecyclePhase.VALIDATE, threadSafe = true, aggregator = true, requiresProject = true)
public class DiagnosticMojo extends AbstractTestOrderMojo {
	...
	public void execute() throws MojoExecutionException {
		...
		if (failOnError && report.hasErrors()) {
			throw new MojoExecutionException("[test-order] Diagnostic found "
					+ report.results().stream().filter(r -> r.isError()).count() + " error(s). "
