...
/**
 * TestRunner implementation that executes tests via Maven Surefire in a subprocess.
 * Uses the test-order {@code FixedOrderClassOrderer} to control execution order precisely.
 * Parses Surefire XML reports to determine pass/fail status.
 */
class MavenTestRunner implements TestRunner {
    ...
    /** Common plugin skip flags to prevent unrelated failures blocking test execution. */
    private static final List<String> PLUGIN_SKIP_FLAGS = List.of(
            "-Drat.skip=true",
    ...
    public boolean runLearnPhase(String instrumentationMode) {
        ...
        List<String> command = new ArrayList<>(List.of(
                "mvn", "me.bechberger:test-order-maven-plugin:learn", "test",
    ...
    public TestRunResult run(List<String> testOrder) {
        ...
        try {
            ...
            // contain identically-named test classes (e.g., com.a.FooTest and com.b.FooTest).
            ...
            List<String> command = new ArrayList<>(List.of(
                    ...
                    "-Dmaven.test.additionalClasspath=" + runtimeDir.toAbsolutePath(),
                    "--batch-mode", "--quiet"));
            command.addAll(PLUGIN_SKIP_FLAGS);
            if (!runAll) {
            ...
            // For multi-module projects, use -pl from parent; for standalone, run in project dir
            File workDir;
            if (project.getBasedir().getParentFile() != null
                    && new File(project.getBasedir().getParentFile(), "pom.xml").exists()
                    && !project.isExecutionRoot()) {
    ...
    private TestRunResult parseResults(List<String> executionOrder) {
        ...
        // Tests not mentioned in reports → consider them passed (they may have been skipped)
    ...
    private void parseMethodReport(Path report, String testClass,
                                   Set<String> passed, Set<String> failed) {
        try {
            String content = Files.readString(report);
            ...
            while (true) {
                ...
                if (methodName != null) {
                    ...
                    // Check if there's a <failure> or <error> child before the next </testcase> or next <testcase
