...
public class DownloadMojo extends AbstractTestOrderMojo {
	...
	public void execute() throws MojoExecutionException {
		...
		// Use reactor root for config lookup (config lives at project root, not per-module)
		...
		if (!me.bechberger.testorder.ci.CiConfigParser.configExistsIn(reactorRoot)) {
			throw new MojoExecutionException(
					"CI download failed: .test-order/download-config.yml not found.\n"
		...
		if (result.isPresent()) {
			...
			throw new MojoExecutionException(
					"CI download failed: could not retrieve artifact from your CI provider.\n"
							+ "  Check:\n"
