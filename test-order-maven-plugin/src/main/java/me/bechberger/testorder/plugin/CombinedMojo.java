 ...
 * <li>Write remaining tests to a file for a separate {@code run-remaining}
 * invocation</li>
 ...
 * <ul>
 * <li>The first {@code test} execution runs only selected tests
 * (fail-fast)</li>
...
public class CombinedMojo extends AbstractTestOrderMojo {
	 ...
	 * auto-detected source packages). Useful if classes are in non-standard
	 * packages.
	 */
	 ...
	 * project groupId for instrumentation. Set to false to only instrument
	...
	/**
	 * Instrumentation mode: METHOD_ENTRY, FULL (default), FULL_METHOD, or
	 * FULL_MEMBER.
	 */
	...
	/**
	 * Number of top-scored test classes to always include in the fast subset.
	 * Top-scored tests are most likely to find regressions.
	 */
	@Parameter(property = MavenPluginConfigKeys.SELECT_TOP_N, defaultValue = "20")
	private int topN;

	/**
	 * Number of random fast tests to include for coverage diversity. Adds
	 * randomness to catch edge cases not covered by top-scored tests.
	 */
	@Parameter(property = MavenPluginConfigKeys.SELECT_RANDOM_M, defaultValue = "10")
	private int randomM;

	/**
	 * Seed for randomM selection. When set, ensures deterministic random selection
	...
	/**
	 * Path to file containing list of deferred tests. These tests were not run in
	 * this invocation and can be run separately via 'run-remaining' goal.
	 */
	...
	/**
	 * Path to file containing list of selected tests. Useful for CI/CD integration
	 * and debugging test selection.
	 */
	...
	/**
	 * Optimise scoring weights every N successful test runs (0 = never, default =
	 * every 10 runs). Weight optimization improves prediction accuracy over time.
	 */
	...
	public void execute() throws MojoExecutionException {
		...
		if (skip)
			return;
		// Guard against double execution (CLI invocation + POM-bound lifecycle
		// execution)
		if ("true".equals(project.getProperties().getProperty("testorder.combined.active"))) {
			getLog().debug("[test-order] Combined mojo already executed in this session — skipping.");
			return;
		}
		...
		// If the index is empty or corrupted (no tests found at all), fall back to
		// learn mode
		if (depMap.size() == 0) {
			getLog().warn("[test-order] Dependency index contains no matching tests — "
					+ "falling back to learn mode. Delete " + idxPath + " to reset.");
			String effectiveInclude = resolveIncludePackages(includePackages, filterByGroupId, project, getLog());
			configureLearnMode(instrumentationMode, effectiveInclude, true);
			return;
		}
		...
		getLog().info("[test-order] Selected " + selection.selected().size() + " tests (fail-fast), deferred "
		...
		// Preserve legacy project property name for compatibility with existing
		// tooling.
		...
		if (optimizeEvery > 0) {
			if (state.runsSinceLearn() > 0 && state.runsSinceLearn() % optimizeEvery == 0) {
				getLog().info(
						"[test-order] Triggering periodic weight optimisation (every " + optimizeEvery + " runs)…");
	...
	private void validateCombinedMojoParameters() throws MojoExecutionException {
		...
		if (topN < 0) {
			throw new MojoExecutionException("[test-order] selectTopN cannot be negative: " + topN);
		}
		...
		if (topN == 0 && randomM == 0) {
			getLog().warn("[test-order] Both selectTopN and selectRandomM are 0 — no tests will be selected. "
