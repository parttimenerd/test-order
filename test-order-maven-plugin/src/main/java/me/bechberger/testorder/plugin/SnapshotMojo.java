...
/**
 * Scans the source tree and saves a compressed file hash snapshot for
 * future "since-last-run" change detection.
 */
@Mojo(name = "snapshot")
public class SnapshotMojo extends AbstractTestOrderMojo {
    ...
    public void execute() throws MojoExecutionException {
        initContext();
        snapshot(ChangeDetectionHelper.resolveSourceRoot(project, sourceRoot),
    ...
    private void snapshot(Path sourceRoot, Path outputFile) throws MojoExecutionException {
        if (!sourceRoot.toFile().isDirectory()) return;
        try {
            FileHashStore store = FileHashStore.scan(sourceRoot);
            store.save(outputFile);
            getLog().info("[test-order] Snapshot: " + store.getHashes().size()
                    + " files (" + sourceRoot + ") → " + outputFile);
        } catch (IOException e) {
