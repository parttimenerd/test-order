...
/**
 * Shared utility methods for locating and manipulating the Surefire plugin configuration.
 */
final class SurefireHelper {
     ...
     */
    static void validateNoClassLevelParallel(MavenProject project, Log log)
        ...
        if (isClassLevelSurefireParallel(surefireParallel)) {
            throw new MojoExecutionException("Unsupported Surefire parallel mode '<parallel>"
                    + surefireParallel + "</parallel>': class-level parallel execution is not supported by test-order. "
     ...
     * contain a Maven property placeholder ({@code ${argLine}} or {@code @{argLine}})
     * and does not already contain the test-order agent. In this case the plugin
     * must inject directly into the Xpp3Dom node instead of relying on property
     * expansion.
     */
    static boolean isHardcodedArgLine(String argLine) {
        if (argLine == null || argLine.isBlank()) return false;
        return !argLine.contains("${argLine}")
            && !argLine.contains("@{argLine}")
            && !argLine.contains("test-order-agent");
    }

    /**
     * Configures Surefire to run only the given test class FQCNs by setting the
     * {@code test} property on both the plugin configuration and the project properties.
     * This maps to Surefire's {@code -Dtest=...} parameter which is always honored.
     *
     * @param clearExisting if true, replaces any previous test filter; if false, appends
     */
    static void configureIncludes(MavenProject project, List<String> tests, boolean clearExisting)
        ...
        for (String tc : tests) {
            if (!testParam.isEmpty()) testParam.append(",");
            testParam.append(tc);
        }
