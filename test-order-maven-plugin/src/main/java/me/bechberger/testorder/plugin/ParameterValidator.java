...
public class ParameterValidator {
    ...
    /**
     * Validates changeMode parameter value.
    ...
    public void validateChangeMode(String changeMode) throws MojoExecutionException {
        ...
        if (!validModes.contains(changeMode.toLowerCase())) {
            throw new MojoExecutionException(
                String.format(
                    "[test-order] Invalid changeMode '%s'. Valid values are: %s",
                    changeMode,
    ...
    /**
     * Validates instrumentationMode parameter value.
    ...
    public void validateInstrumentationMode(String instrumentationMode) throws MojoExecutionException {
        ...
        Set<String> validModes = new HashSet<>(Arrays.asList("FULL", "SMART"));

        if (!validModes.contains(instrumentationMode.toUpperCase())) {
            throw new MojoExecutionException(
                String.format(
                    "[test-order] Invalid instrumentationMode '%s'. Valid values are: %s",
                    instrumentationMode,
                    String.join(", ", validModes)
                )
            );
    ...
    /**
     * Validates outputFormat parameter value.
    ...
    public void validateOutputFormat(String outputFormat) throws MojoExecutionException {
        ...
        if (!validFormats.contains(outputFormat.toLowerCase())) {
            throw new MojoExecutionException(
                String.format(
                    "[test-order] Invalid outputFormat '%s'. Valid values are: %s",
                    outputFormat,
    ...
    public void validateFilePath(String filePath, String parameterName) throws MojoExecutionException {
        ...
        if (!Files.isReadable(path)) {
            throw new MojoExecutionException(
                String.format(
                    "[test-order] %s file is not readable: %s",
                    parameterName,
                    path.toAbsolutePath()
                )
            );
    ...
    public void validateOutputDirectory(String dirPath, String parameterName) throws MojoExecutionException {
        ...
        if (!Files.exists(path)) {
            try {
                ...
                throw new MojoExecutionException(
                    String.format(
                        "[test-order] Failed to create %s directory: %s",
                        parameterName,
                        path.toAbsolutePath()
                    ),
                    e
        ...
        if (!Files.isDirectory(path)) {
            throw new MojoExecutionException(
                String.format(
                    "[test-order] %s is not a directory: %s",
                    parameterName,
                    path.toAbsolutePath()
                )
            );
        ...
        if (!Files.isWritable(path)) {
            throw new MojoExecutionException(
                String.format(
                    "[test-order] %s directory is not writable: %s",
                    parameterName,
                    path.toAbsolutePath()
                )
            );
    ...
    public void validateIntRange(int value, int min, int max, String parameterName) throws MojoExecutionException {
        if (value < min || value > max) {
            throw new MojoExecutionException(
                String.format(
                    "[test-order] %s value %d is out of range [%d, %d]",
                    parameterName,
                    value,
                    min,
                    max
