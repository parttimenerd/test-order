...
public class AutoMojo extends AbstractTestOrderMojo {
	...
	public void execute() throws MojoExecutionException {
		...
		if (isPrepareGoalBound()) {
			getLog().warn(
					"[test-order] The 'prepare' goal is bound" + " in your POM but you are running 'auto' on the CLI."
							+ " Both configure Surefire and may conflict.");
		}
		...
		if (result instanceof AutoWorkflow.Result.Skip s) {
			...
			if (os.selectResult().allSelected()) {
				getLog().info("[test-order] Running full test suite" + " (selection covered all tests)");
