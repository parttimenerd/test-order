...
/**
 * JUnit MethodOrderer that reorders test methods within a class based on fail-fast scores.
 * <p>
 ...
 *     <li><b>Speed (class-local):</b> Fast methods run before slow ones, compared to the class median.</li>
 * </ul>
 * <p>
 * <b>Graceful degradation:</b> If no method telemetry is available (first run), methods remain
 * in source order. This is logged, not an error.
 * <p>
 * Methods with explicit {@code @Order} annotations take precedence over score-based ordering.
 */
public class PriorityMethodOrderer implements MethodOrderer {
     ...
     */
    public static synchronized void setPendingState(TestOrderState state,
    ...
    public void orderMethods(MethodOrdererContext context) {
        ...
        for (Method m : methods) {
            ...
            methodMetadata.add(new MethodScorer.MethodMetadata(
                    className, methodName, (long) duration, null));
        }

        // Graceful fallback: if no telemetry at all (no durations, no failures), keep source order
        boolean hasDurations = false;
        boolean hasFailures = false;
        for (MethodScorer.MethodMetadata m : methodMetadata) {
            if (m.durationMs() >= 0) hasDurations = true;
            if (pendingState.methodFailureScore(m.className(), m.methodName()) > 0) hasFailures = true;
        ...
        // Score methods
        MethodScorer scorer = new MethodScorer(methodWeights, pendingState,
                depMap, changedClasses, changedMethods);
        ...
        for (MethodScorer.MethodScoreResult score : scores) {
            TestOrderLogger.debug("[method-order] → {}: score={} (recency={}, speed={}, depOverlap={}, coverage={}, new={}, changed={}, classMedian={}ms)",
                    ...
                    String.format("%.1f", score.speedBonus()),
                    String.format("%.1f", score.depOverlapBonus()),
                    String.format("%.1f", score.coverageBonus()),
                    String.format("%.1f", score.newMethodBonus()),
                    String.format("%.1f", score.changedMethodBonus()),
                    score.classMedianMs());
        }

        // Pre-build lookup map for O(1) score access (instead of O(N) linear search per comparator call)
        Map<String, Integer> scoreIndexMap = new HashMap<>(scores.size() * 2);
        for (int i = 0; i < scores.size(); i++) {
            scoreIndexMap.put(scores.get(i).methodName(), i);
        }

        // Sort by score descending (highest score runs first), maintain source order for ties
