 ...
 * 
 * When Maven runs tests in parallel (with multiple threads or forked processes),
 * test-order must handle concurrent state file writes correctly:
 * - No data loss when multiple threads write timing/scoring data
 * - Correct test count tracking despite race conditions
 * - Accurate duration tracking even with thread interleaving
 * 
...
public class ParallelExecutionIT extends BaseFixtureIT {
    ...
    void testParallelForkExecution(@TempDir Path tempDir) throws Exception {
        ...
        // Verify output shows tests ran (can't verify exact order with forking, but can verify count)
        assertOutputContains(output, "Concurrent Execution - Suite A",
            "Suite A tests should be reported");
        assertOutputContains(output, "Concurrent Execution - Suite B",
            "Suite B tests should be reported");
        assertOutputContains(output, "Concurrent Execution - Suite C",
            "Suite C tests should be reported");
    }
    ...
    void testConcurrentStateFileWrites(@TempDir Path tempDir) throws Exception {
        ...
        int testCount = getTestCount(output);
        assertEquals(18, testCount,
            "State file corruption would manifest as missing/extra test counts");
    ...
    // Helper method to do basic assertion (since BaseFixtureIT provides assertEquals)
