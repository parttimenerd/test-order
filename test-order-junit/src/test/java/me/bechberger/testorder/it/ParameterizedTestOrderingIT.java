...
/**
 * Integration test for parameterized tests.
 * Validates that test-order correctly handles:
 * - @ParameterizedTest with @CsvSource
 * - @ParameterizedTest with @ValueSource
 * - Duration aggregation across parameter sets
 * - Method-level scoring with dynamic test IDs [index]
 */
public class ParameterizedTestOrderingIT extends BaseFixtureIT {
    ...
    public void testParameterizedTestsFull(@TempDir Path tempDir) throws Exception {
        ...
        // 5 CSV tests (add) + 3 CSV tests (subtract) + 3 CSV tests (multiply) + 8 value tests (prime true) + 8 value tests (prime false) = 27
    ...
    public void testDurationAggregationForParameterized(@TempDir Path tempDir) throws Exception {
        ...
        // Duration tracking should aggregate all 27 test instances into their method-level buckets
