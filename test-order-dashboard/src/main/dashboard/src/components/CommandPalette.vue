...
const ACTIONS: ActionItem[] = [
  ...
  { type: 'action', icon: '⊕', label: 'Filter: dep-overlap', desc: 'Show only tests with dependency overlap on changed classes', keywords: ['filter', 'dep', 'overlap', 'dependency', 'changed'], run: () => d.setBadgeFilter('depoverlap') },
  { type: 'action', icon: '⚡', label: 'Filter: static-field overlap', desc: 'Show only tests that read static fields of changed classes', keywords: ['filter', 'static', 'field', 'stat', 'changed'], run: () => d.setBadgeFilter('static') },
  { type: 'action', icon: '📈', label: 'Filter: high variance', desc: 'Show only tests with high duration variance (CV≥50%)', keywords: ['filter', 'variance', 'unstable', 'timing', 'high'], run: () => d.setBadgeFilter('variance') },
  { type: 'action', icon: '⊙', label: 'Filter: affected by changes', desc: 'Show tests with dep-overlap, changed source, or new (tier-1 selection)', keywords: ['filter', 'affected', 'changed', 'dep', 'new', 'tier1'], run: () => d.setBadgeFilter('affected') },
  { type: 'action', icon: '⚙', label: 'Filter: has method data', desc: 'Show only tests with method-level dependency data (is:method)', keywords: ['filter', 'method', 'methods', 'dep', 'method-level'], run: () => { d.setBadgeFilter(null); d.searchQ.value = 'is:method' } },
  { type: 'action', icon: '⚠', label: 'Filter: high ML risk', desc: 'Show only tests ML predicts >50% failure probability (is:risk)', keywords: ['filter', 'ml', 'risk', 'fail', 'predict', 'probability'], run: () => { d.setBadgeFilter(null); d.searchQ.value = 'is:risk' } },
  { type: 'action', icon: '⊘', label: 'Clear filter', desc: 'Remove the active filter', keywords: ['clear', 'filter', 'reset', 'all'], run: () => { d.setBadgeFilter(null); d.searchQ.value = '' } },
