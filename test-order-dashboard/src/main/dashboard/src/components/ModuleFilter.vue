<template>
  <div v-if="modules.length > 1" class="module-filter">
    ...
    <div v-if="modelValue && count != null" class="module-filter__count">{{ count }} test{{ count === 1 ? '' : 's' }}</div>
  </div>
</template>
...
<style scoped>
.module-filter { margin-bottom: 8px; }
...
.module-filter__clear { background: none; border: none; cursor: pointer; color: var(--text-secondary, #888); font-size: 14px; line-height: 1; padding: 0 2px; }
...
.module-filter__chip:hover { background: var(--accent-dim, #3a3a4a); color: var(--text-primary, #eee); }
.module-filter__chip--active { background: var(--accent, #5b8dd9); color: #fff; border-color: var(--accent, #5b8dd9); }
...
.module-filter__count { font-size: 11px; color: var(--text-secondary, #888); margin-top: 4px; }
</style>
