 ...
 *
 * <h3>Data layout</h3>
 * Class IDs (0 .. MEMBER_ID_OFFSET-1) and member IDs (MEMBER_ID_OFFSET ..)
 * are stored in <em>separate</em> {@code volatile long[]} arrays.  Keeping them
 * apart means member IDs are stored at their <em>adjusted</em> index
 * (memberId - MEMBER_ID_OFFSET), so both arrays stay small:
 * <ul>
 ...
 *   <li>Otherwise → one {@code lock or} via {@code VarHandle.getAndBitwiseOr}.</li>
 * </ol>
 * The speculative check is safe because bits are <em>monotone</em> (0 → 1 only):
 * seeing a stale "1" is impossible; seeing a stale "0" at most causes an
...
public class BitsetTracker {
    ...
    // 512 words × 64 = 32 768 member-ID slots (4 KB); covers 4 096 classes × ~8 members
    private static final int INITIAL_MEMBER_WORDS = 512;

    /**
     * Atomic bitset backed by a volatile long[].
     * Growth replaces the array under a lock; reads and bit-sets are lock-free.
     */
    private static final VarHandle HIGHWATER;
    static {
        try {
            HIGHWATER = MethodHandles.lookup()
                    .findVarHandle(WordArray.class, "highWater", int.class);
    ...
    private static final class WordArray {
        volatile long[] words;
        /** High-water mark: highest word index written to.  Updated atomically
         *  via CAS to prevent races where a smaller index overwrites a larger one.
         *  Used at flush time to limit iteration range. */
        ...
        private void advanceHighWater(int index) {
            ...
            while (index > (cur = highWater)) {
                if (HIGHWATER.compareAndSet(this, cur, index)) return;
            }
        }
        ...
        int count() {
            ...
            for (int i = 0; i < limit; i++) n += Long.bitCount(w[i]);
            return n;
        }
    ...
    /** Convert recorded class IDs to class names. Single-pass: iterates only
     *  up to the high-water mark and skips zero words. */
    public java.util.Set<String> toClassNames() {
        int hw = classWords.highWater;
        if (hw < 0) return java.util.Collections.emptySet();
        ...
        int estimated = 0;
        for (int wi = 0; wi < limit; wi++) estimated += Long.bitCount(w[wi]);
    ...
    /** Convert recorded member IDs to member names. Single-pass: iterates only
     *  up to the high-water mark and skips zero words. */
