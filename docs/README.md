...
| [GETTING_STARTED.mdx](GETTING_STARTED.mdx) | Step-by-step tutorial: first run → reordering → dashboard |
...
| [CLI_REFERENCE.mdx](CLI_REFERENCE.mdx) | Goals, properties, change-detection modes, ML configuration, CI patterns |
...
| [MULTI_MODULE_SETUP.mdx](MULTI_MODULE_SETUP.mdx) | Multi-module project setup and aggregation |
| [SCORING.md](SCORING.md) | Scoring formula, weights, ML-enhanced scoring, and automatic tuning |
| [FLAKY_AND_CACHING.mdx](FLAKY_AND_CACHING.mdx) | Auto-retry FLAKY tests, quarantine mode, and skip-if-unchanged caching |
...
- **New here?** Start with the [Cheat Sheet](CHEAT_SHEET.md) or the [Getting Started tutorial](GETTING_STARTED.mdx)
...
- **Dashboard**: [CLI_REFERENCE.mdx § Dashboard](CLI_REFERENCE.mdx#dashboard)

## Where do I start?

Pick the path that matches your role:

- **New user, just want to try it** → [GETTING_STARTED.mdx](GETTING_STARTED.mdx), then poke at [`samples/sample-basic`](https://github.com/parttimenerd/test-order/tree/main/samples/sample-basic).
- **Using TestNG** → [GETTING_STARTED.mdx](GETTING_STARTED.mdx) applies equally; see [FRAMEWORK_COMPARISON.md](FRAMEWORK_COMPARISON.md) for differences from the JUnit integration.
...
- **Multi-module reactor maintainer** → [MULTI_MODULE_SETUP.mdx](MULTI_MODULE_SETUP.mdx).
