...
> **Upgrading from an earlier snapshot?** Check [CHANGELOG.md](https://github.com/parttimenerd/test-order/blob/main/CHANGELOG.md) before updating. The current unreleased version contains breaking changes: the property namespace `testorder.select.*` was renamed to `testorder.affected.*` (e.g. `-Dtestorder.select.topN` → `-Dtestorder.affected.topN`), and the Gradle task `testOrderSelect` was renamed to `testOrderAffected`. Old names produce an "unknown property" warning with a suggested replacement.
...
| Tests start failing after enabling test-order | Pre-existing test isolation bug exposed by reordering — tests that share static state, temp files, or DB rows may only fail in certain orders | Run `mvn test-order:auto test -Dtestorder.shuffle=true` a few times to reproduce; then use [DETECT_DEPENDENCIES.md](DETECT_DEPENDENCIES.md) to find which tests pollute others |
| `-Dtest=...` filter active but test-order ignores it / no reordering | When `-Dtest` is set, test-order deliberately skips auto-selection and delegates entirely to Surefire (logged at INFO). This is intentional — test-order won't override your explicit filter. | Run without `-Dtest` to get auto-selection, or use `-Dtestorder.mode=order` with `-Dtest` to get ordering-only without selection |
