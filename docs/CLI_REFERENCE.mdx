...
| `optimize` | Run the scoring weight optimizer against the stored run history, then save the optimised weights to the state file. Does not run tests. |
...
# Run weight optimizer and update state file (does not run tests)
mvn test-order:auto -Dtestorder.mode=optimize
...
All three features below are opt-in and independent of each other. See [FLAKY_AND_CACHING.mdx](FLAKY_AND_CACHING.mdx) for the full guide.
...
When something looks off, `diagnose` checks index health, agent attachment, framework detection, and configuration issues.
