...
/**
 * Manages caching of downloaded CI artifacts.
 * Stores artifacts in .test-order-cache/ with metadata (timestamp, source, checksum).
 * Provides cleanup and version tracking capabilities.
 */
public class ArtifactCache {
    ...
    public Optional<Path> getLatestCached(String artifactName) {
        return metadata.values().stream()
            .filter(e -> e.getName().equals(artifactName))
            .sorted(Comparator.comparing(CacheEntry::getTimestamp).reversed())
            .findFirst()
