...
class DownloaderTests {
    ...
    void testGitHubConfigValidation() throws DepDownloader.DepDownloadException {
        CiConfig.GithubConfig validConfig = new CiConfig.GithubConfig(
            "bechberger", "test-order", "integration-tests.yml", "test-order-deps", "main"
        );
    ...
    void testGitHubConfigValidationMissingOwner() {
        CiConfig.GithubConfig invalidConfig = new CiConfig.GithubConfig(
            null, "test-order", "workflow.yml", "artifact", "main"
        );
    ...
    void testGitHubDownloaderName() {
        CiConfig.GithubConfig config = new CiConfig.GithubConfig(
            "owner", "repo", "workflow", "artifact", "main"
        );
    ...
    void testHttpConfigValidation() throws DepDownloader.DepDownloadException {
        CiConfig.HttpConfig validConfig = new CiConfig.HttpConfig(
            "https://example.com/artifacts/deps.json", "bearer", "TOKEN"
        );
    ...
    void testHttpConfigValidationMissingUrl() {
        CiConfig.HttpConfig invalidConfig = new CiConfig.HttpConfig(
            null, "bearer", "TOKEN"
        );
    ...
    void testHttpDownloaderName() {
        CiConfig.HttpConfig config = new CiConfig.HttpConfig(
            "https://example.com/deps", "bearer", null
        );
