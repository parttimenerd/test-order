 ...
 * Tests how test-order scales with many classes, tests, and dependencies.
 */
class LargeCodebasePerformanceTest {
    ...
    void testLargeNumberOfClasses(@TempDir Path workDir) throws Exception {
        ...
        for (int i = 0; i < 1000; i++) {
            Path classFile = project.resolve(String.format(
                "src/main/java/com/example/Service%d.java", i));
            String classCode = String.format(
                "package com.example; public class Service%d { public void process() {} }",
                i);
            Files.writeString(classFile, classCode);
        }
        
        long classCount = Files.walk(project.resolve("src/main/java"))
    ...
    void testLargeNumberOfTestMethods(@TempDir Path workDir) throws Exception {
        ...
        for (int i = 0; i < 100; i++) {
            Path testFile = project.resolve(String.format(
                "src/test/java/com/example/Test%d.java", i));
        ...
        long testMethodCount = Files.walk(project.resolve("src/test/java"))
            .filter(p -> p.toString().endsWith(".java"))
            .count();
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (i == 0) {
                ...
                classCode = String.format(
                    "package com.example; public class Class%d { " +
                    "  Class%d dep; " +
                    "  public void process() { dep.process(); } " +
                    "}",
                    i, i - 1);
            }
    ...
    void testWideDependencyGraphs(@TempDir Path workDir) throws Exception {
        ...
        for (int i = 0; i < 1000; i++) {
            Path classFile = project.resolve(String.format(
                "src/main/java/com/example/Dependent%d.java", i));
            String classCode = String.format(
                "package com.example; public class Dependent%d { " +
                "  void use() { CoreUtility.format(this); } " +
                "}", i);
            Files.writeString(classFile, classCode);
        }
        
        long dependentCount = Files.walk(project.resolve("src/main/java"))
            .filter(p -> p.getFileName().toString().startsWith("Dependent"))
            .count();
    ...
    /**
     * Test: Handle projects with multiple language support (Java + Kotlin + Groovy).
     */
    @Test
    ...
    void testStateFileGrowth(@TempDir Path workDir) throws Exception {
        ...
        for (int run = 0; run < 100; run++) {
            String stateData = String.format(
                "run %d timestamp %d classes %d\n",
                run, System.currentTimeMillis(), 100 + run);
            Files.write(stateFile, stateData.getBytes(),
    ...
    void testCacheMemoryEfficiency(@TempDir Path workDir) throws Exception {
        ...
        for (int i = 0; i < cacheSize; i++) {
            cache.put(
                "dependency-" + i,
                "class-hash-" + Integer.toHexString(i));
        }
