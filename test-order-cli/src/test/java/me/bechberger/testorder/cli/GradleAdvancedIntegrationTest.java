...
/**
 * Advanced Gradle integration tests.
 * Tests Gradle plugin integration, configuration options, and real-world scenarios.
 */
class GradleAdvancedIntegrationTest {
    ...
    void testGradleMultiModuleProject(@TempDir Path workDir) throws Exception {
        ...
        assertEquals(3, Files.list(project)
            .filter(p -> Files.isDirectory(p))
            .filter(p -> Files.exists(p.resolve("build.gradle")))
            .count(), "Should have 3 modules");
    }
    ...
    void testGradleWithTestorder(@TempDir Path workDir) throws Exception {
        ...
        String withTestOrder = buildGradle.replace(
            "plugins {",
    ...
    void testGradleBuildCacheIntegration(@TempDir Path workDir) throws Exception {
        ...
        assertTrue(Files.exists(project.resolve("gradle.properties")), 
    ...
    private void createGradleModule(Path moduleDir, String moduleName) throws Exception {
        ...
        String javaClass = String.format(
            "package com.example; public class %s { }",
            capitalize(moduleName));
        Files.writeString(
            moduleDir.resolve("src/main/java/com/example/" + capitalize(moduleName) + ".java"),
            javaClass);
    }
