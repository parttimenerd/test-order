...
/**
 * Integration tests for multi-module Maven projects. Tests change detection,
 * test selection, and coverage across multiple modules.
 */
class MultiModuleProjectTest {

	/**
	 * Test: Detect changes in one module, run tests only for that module and
	 * dependents.
	 */
	@Test
	...
	/**
	 * Test: Test exclusions in multi-module (skip slow/integration tests in some
	 * modules).
	 */
	@Test
	...
	void testCrossModuleDependencyIndex(@TempDir Path workDir) throws Exception {
		...
		// Simulation: when interface module changes, impl module tests should be
		// selected
	...
	private void createModuleStructure(Path module, String moduleName) throws Exception {
		...
		String testCode = String.format("package com.example; import org.junit.jupiter.api.Test; "
				+ "class ServiceTest { @Test void test() {} }", moduleName);
